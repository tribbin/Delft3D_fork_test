...
\Note a common issue is "Activate.ps1 is not digitally signed. You cannot run this script on the current system." Look here on how to troubleshoot: \autoref{sec:troubleshooting}
...
\item[Activate.ps1 is not digitally signed] If the activate command for the python virtual enviroment generates the message "Activate.ps1 is not digitally signed. You cannot run this script on the current system.", then you need to temporarily change the PowerShell execution policy to allow scripts to run: \command{Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process}
