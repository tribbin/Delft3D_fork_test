...
from tools.minio.error import MinioAuthError, MinioCliArgsError, MinioToolError
...
try:
    ...
    exit(1)
except MinioCliArgsError as exc:
    print(f"Error: {exc.message.rstrip('.')}.", file=sys.stderr)
    for key, value in exc.cli_args.items():
        print(f"{key}: {'missing' if value is None else str(value)}", file=sys.stderr)
    exit(1)
except MinioAuthError as exc:
