...
year_to_replace = "[12][09][0-2][0-9]"  # The year to replace in the copyright
...
regex_to_search = [re.compile(f"(?P<before>Copyright.*Deltares.*[-\W+])({year_to_replace})(?P<after>[^-].*)", re.IGNORECASE),
                   re.compile(f"(?P<before>Copyright.*[-\W+])({year_to_replace})(?P<after>[^-].*Deltares.*)", re.IGNORECASE)]
...
for idx, file in enumerate(file_list, 1):
    print(f"\rProcessing file {idx} of {total_files}".rjust(100), end="")
...
print(f"\nProcessing complete. Elapsed time: {elapsed_time:.2f} seconds")
