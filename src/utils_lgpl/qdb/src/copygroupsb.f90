...
!
!-------------------------------------------------------------------------------
subroutine copygroupsb(fds1, fds2, fds3, grp, in_idx1, in_idx2, in_idx3, fout, mlim, nlim)
   !
   !=============================================================
   ...
   ! FDS1 = SMALL / FDS2 = FDS3 = BIG
   ! In case of a free dimension, the index IN_IDX1 is read from
   ! file FDS1 and stored as IN_IDX2 on file FDS2. The indices can be:
   !
...
!    mmax2 = mlim(2)-mlim(1)+1
!    nmax2 = nlim(2)-nlim(1)+1
...
!             endif
   ...
   do i = 1, nelems
      ...
      if (ierr /= 0) goto 9999
      !
      !      ierr = inqelm (fds1, elmnms(i), elmtyp, bytes, elmqty, elmunt, elmdes, elmndm, elmdms2)
      !      if (ierr /= 0) goto 9999
      !
      buflen = bytes * grpsz
      do d = 1, elmndm
...
!           write (*,*) 'REAL ',i, elmnms(i)
            ...
            if (ierr /= 0) goto 9999
!                   write (*,*) elmdms(1), elmdms(2),elmdms(3), elmdms(4), elmdms(5)
            rarray(nlim(1) + 1:nlim(2) - 1, mlim(1) + 1:mlim(2) - 1, :, :, :) = rarray2(2:nmax2 - 1, 2:mmax2 - 1, :, :, :)
            !         write(*,*) 'PASSED'
!                   write(*,*) rarray(5:8,72,1,1,1)
!                   write(*,*) rarray2(5:8,18,1,1,1)
         ...
         elseif (elmtyp(1:7) == 'INTEGER') then
            ...
            if (ierr /= 0) goto 9999
!               iarray(nlim(1):nlim(2), mlim(1):mlim(2), :, :, :) = iarray2
            iarray(nlim(1) + 1:nlim(2) - 1, mlim(1) + 1:mlim(2) - 1, :, :, :) = iarray2(2:nmax2 - 1, 2:mmax2 - 1, :, :, :)
