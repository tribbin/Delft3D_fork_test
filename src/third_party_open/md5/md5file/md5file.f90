...
!> generates a MD5 checksum for input array
   subroutine md5intarr(intarr, checksum, success)
      ...
      integer :: no_chunks
      integer :: i, ii, j, k, m
      character(len=c_sizeof(i)) :: datablock

      success = .true.

      array_size = size(intarr) * c_sizeof(i)
      ...
      do i = 1, no_chunks
         ...
         do j = 1, length / c_sizeof(i)
            dataBlock(1:c_sizeof(i)) = transfer(intarr(ii + j), repeat(' ', c_sizeof(i)))

            do k = 1, c_sizeof(i)
               m = c_sizeof(i) * j + k - c_sizeof(i)
               chunk(m:m) = dataBlock(k:k)
            end do
         end do
         ii = ii + length / c_sizeof(i)
