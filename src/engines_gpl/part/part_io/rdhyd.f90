...
contains
    ...
    subroutine rdhyd  (nfiles, lunit, fnam, hyd, num_layers, zmodel, ihdel, &
        ...
        if (ipath == 0) ipath = scan(fnam(18), "/", .true.)
        ...
        do while (error == 0)
            ...
            case ("pointers-file")
                read (line, *) token1, token2
                if (token2 == 'none' .or. ipath == 0) then
                    fnam(19) = token2
                else
            ...
            case ("domains")
                ...
                i = 0
                do while (token2 /= "end-domains")
                    read  (lunit(18), '(A)') line
                    read  (line, *) token2
                    if (token2 /= "end-domains") then
                        i = i + 1
                        if (i > ndoms) then
            ...
            case ("dd-boundaries")
                i = 0
                do while (token2 /= "end-dd-boundaries")
                    read  (lunit(18), '(A)') line
                    read  (line, *) token2
                    if (token2 /= "end-dd-boundaries") then
                        i = i + 1
                        if (i > nbnds) then
                ...
                do i = 1, nbnds              !     Find the corresponding domain numbers
                    ...
                    bnds(i)%r2%did = 0
                    do j = 1, ndoms
                        if (bnds(i)%r1%name == doms(j)%name) bnds(i)%r1%did = j
                        if (bnds(i)%r2%name == doms(j)%name) bnds(i)%r2%did = j
                    enddo
                    if (bnds(i)%r1%did == 0 .or. bnds(i)%r2%did == 0) then
