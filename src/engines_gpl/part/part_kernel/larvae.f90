...
contains
    subroutine larvae (lunrep, itime, idelt, num_rows, num_columns, &
        ...
        if (ifirst == 1) then
            ...
            if (num_constants /= nfix + nstage * nvar) then
        ...
        do ipart = npwndw, nopart
            ...
            if (iseg > 0) then
                ...
                if(k /= 1) laydep = laydep - locdep(lgrid2(n, m), k - 1)
                ...
                stime = wpart(3, ipart)
                stemp = wpart(4, ipart)
                if (nosubs >= 4 + nstage + 1) dtemp = wpart(4 + nstage + 1, ipart)
                if (nosubs >= 4 + nstage + 2) ddepth = wpart(4 + nstage + 2, ipart)
                if (nosubs >= 4 + nstage + 3) sdepth = wpart(4 + nstage + 3, ipart)
                istage = stage
                istage = max(istage, 0)
                istag0 = istage

                stemp = (stemp * stime + temp * delt) / (stime + delt)
                if (nosubs >= 4 + nstage + 1) dtemp = dtemp + temp
                if (nosubs >= 4 + nstage + 2) ddepth = ddepth + zdepth
                if (nosubs >= 4 + nstage + 3) sdepth = (sdepth * stime + zdepth * delt) / (stime + delt)
                stime = stime + delt

                if (istage == 0) then
                    if (stime > -(stage + delt)) then
                        stage = 1.0
                        istage = 1
                        fstage = 0.0
                        stime = 0.0
                ...
                if (istage > 0) then
                    ...
                    if (abs(tcmort(istage)) > 1.e-20) then
                ...
                do istage_t = 1, nstage
                    if (istage_t + 4 <= nosubs) then
                        if (istage_t == istage) then
                ...
                if (istage >= istage_nursery .and. is_nursery) then
                    kpart(ipart) = num_layers + 1
                elseif (istage > 0) then
                ...
                if (nosubs >= 4 + nstage + 1) wpart(4 + nstage + 1, ipart) = dtemp
                if (nosubs >= 4 + nstage + 2) wpart(4 + nstage + 2, ipart) = ddepth
                if (nosubs >= 4 + nstage + 3) wpart(4 + nstage + 3, ipart) = sdepth
