...
contains
    ...
    subroutine findpoly   (num_rows, num_columns, lgrid, lgrid2, xcor, &
        ...
        try = 0
        inside = 0
        do while (inside == 0 .and. try < 1000)
    ...
    subroutine findcell (num_rows, num_columns, xnloc, ynloc, lgrid, &
        ...
        if (ierror /= 0) then ! the method above is faster, but doesn't always work with DD. When the cell was not found use, pinpok for all cells until found
        ...
        if (ierror /= 0) nmloc = 1           ! still not found, so probably outside the grid?!?
