...
subroutine xbeach_wave_compute_flowforcing2D()
   ...
   use m_sferic, only: pi
   use m_xbeach_data
   use fm_external_forcing_data, only: nbndw, kbndw, nbndz, kbndz, nbndu, kbndu
   use m_physcoef, only: ag
...
! account for thin dams
   ...
   use fm_external_forcing_data, only: nbndu
...
subroutine xbeach_solve_wave_stationary(callType,ierr)
   ...
   use fm_external_forcing_data, only: nbndw, kbndw
   use m_alloc
   use unstruc_display
