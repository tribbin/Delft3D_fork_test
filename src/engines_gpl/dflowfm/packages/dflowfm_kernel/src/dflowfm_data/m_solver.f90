...
!     matrix
      ...
      real(kind=dp), dimension(:), allocatable :: a !< matrix entries,   dim(numnonzeros)
...
!     preconditioner
      ...
      real(kind=dp), dimension(:), allocatable :: alu !< precond. matrix entries,   dim(numnonzerosprecond)
...
!     work array
      integer :: nwork
      real(kind=dp), dimension(:), allocatable :: work !< work array, dimension(nwork)     !< size of work array
      integer, dimension(:), allocatable :: jw !< nonzero indicater, dim(2*nrows)

!     settings
      integer, dimension(16) :: ipar
      real(kind=dp), dimension(16) :: fpar
      real(kind=dp) :: alpha
      integer :: lfil
      real(kind=dp) :: tol
      real(kind=dp) :: eps
