...
! COMMON indices for all structure types:
 ...
 type(tgate), allocatable :: gates(:)
   ...
   subroutine init_structure_hisvalues()
      use fm_external_forcing_data , only: npumpsg, ncgensg, ngatesg, ncdamsg, ngategen, ngenstru, nweirgen, ndambreaksignals
...
function get_number_of_structures(struc_type_id) result(number_of_structures)
   use m_GlobalParameters
   use fm_external_forcing_data, only: ncdamsg, ndambreaksignals, ngatesg, npumpsg
   use unstruc_channel_flow, only: network
...
subroutine get_geom_coordinates_of_generalstructure_oldext(i, nNodes, x, y)
   use m_alloc
   use fm_external_forcing_data, only: ncgensg, kcgen, L1cgensg, L2cgensg
   use m_flowgeom, only: lncn
   use network_data, only: xk, yk
   implicit none
   integer,                       intent(in   ) :: i      !< Structure index for this structure type.
...
integer function number_of_pump_nodes
   ...
   use fm_external_forcing_data, only: npumpsg, L1pumpsg, L2pumpsg
   use unstruc_channel_flow, only: network
...
subroutine retrieve_set_of_flowlinks_pump(i_pump, links)
   use fm_external_forcing_data, only: L1pumpsg, L2pumpsg, kpump
...
subroutine retrieve_set_of_flowlinks_dam(i_dam, links)
   use fm_external_forcing_data, only: L1cdamsg, L2cdamsg, kcdam
...
subroutine retrieve_set_of_flowlinks_dambreak(i_dambreak, links)
   use fm_external_forcing_data, only: L1dambreaksg, L2dambreaksg, kdambreak
...
subroutine retrieve_set_of_flowlinks_gate(i_gate, links)
   use fm_external_forcing_data, only: L1gatesg, L2gatesg, kgate
