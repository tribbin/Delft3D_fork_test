...
module m_orthosettings
   ...
   real(kind=dp) :: ATPF = 0.975_dp !< Factor (0.<=ATPF<=1.) between grid smoothing and grid ortho resp.
   real(kind=dp) :: ATPF_B = 1.0_dp !< minimum ATPF on the boundary
   real(kind=dp) :: circumormasscenter = 1.0_dp !< 1.0 = circumcentre,      0.0 = masscentre, 1.0 -> 0.0 : weighted
   real(kind=dp) :: smoothorarea = 1.0_dp !< Factor between smoother (1d0) and area-homogenizer (0d0)
   integer :: adapt_method = 1 !< Mesh-adaptation method; 0: Winslow, 1: arc-length, 2: harmonic map
   real(kind=dp) :: adapt_beta = 0.0_dp !< Mesh-refinement factor; between 0d0 and 1d0
   ...
   real(kind=dp) :: ortho_pure = 0.5_dp !< curvi-linear-like (0d0) or pure (1d0) orthogonalisation
