...
!       Read entire HAV record (maybe several lines) and put it into buffer
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - HAVC ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!       read error handling
   ...
   991  Continue
        ReadError = .true.
   992  Continue
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input HAVC',IOUT1)
     Enddo
 211 CONTINUE
...
!       Read entire HEDG record (maybe several lines) and put it into buffer
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - HEDG ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!       read error handling
   ...
   993  Continue
        ReadError = .true.
   994  Continue
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input HEDG',IOUT1)
     Enddo
2121 CONTINUE
     If (teller .lt. Nrsv) then
         RetVal = 930
...
!       Read record using ParseToken
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - BOTG ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!           Assign definition to possible other reservoirs using the same BOTG definition
          ...
          Endif
        Endif
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input BOTG',IOUT1)
     Enddo
2111 CONTINUE
     If (teller .lt. NRsv) then
         RetVal = 930
...
!       Read using ParseToken
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - TURB ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!           Assign definition to possible other reservoirs using the same TURB definition
          ...
          Endif
        Endif
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input TURB',IOUT1)
     Enddo
2112 CONTINUE
     If (teller .lt. NRsv) then
         RetVal = 930
...
!       Read input using ParseToken
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - SPIL ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!           Assign definition to possible other reservoirs using the same SPIL definition
          ...
          Endif
        Endif
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input SPIL',IOUT1)
     Enddo
2113 CONTINUE
     If (teller .lt. NRsv) then
         RetVal = 930
...
!       Read entire QHRE record (maybe several lines) and put it into buffer
        ...
        if (.not. Success) ReadError = .true.
        IF (ReadError) then
           CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading reservoir datafile - QHRE ParseToken ',IOUT1)
           RetVal = 974
           Return
        Endif
        Name = ' '
...
!       read error handling
   ...
   995  Continue
        ReadError = .true.
   996  Continue
        IF (ReadError) CALL ERRMSG2 (974,0,'ReadRsv',' Read error during reading RTC Reservoir input QHRE',IOUT1)
...
! Taiwan Nov2003: FullRsvLevel = MAX of spillway outlet
     ...
     150 CONTINUE
         CALL ERRMSG2 (974, 1, ' Read error in ReadReservoirInput',' ',Iout1)
         RetVal = 974
