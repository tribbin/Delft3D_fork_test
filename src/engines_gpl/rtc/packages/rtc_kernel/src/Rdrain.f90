...
! *********************************************************************
      ...
      IF (ENDFIL) then
         CALL ERRMSG2 (911, 0, 'Rdrain', ' Precipitation file', IOUT1)
         Retval = 911
         Return
      Endif
...
! Zet definitieve koppeling stations aan precip data
      Do IPre = 1,NPrecp
         ...
         if (loc2Stat(ipre) .eq. -1) then
            CALL ERRMSG2 (938, 0, '  Rdrain', Id_pre(ipre), IOUT1)
            Retval = 938
            Return
         Endif
      Enddo
...
! *********************************************************************
      ...
      DO 850 IEVENT=1,NEVENT
        ...
        CALL SKPCOM (IN, ENDFIL, 'RTC')
        IF (ENDFIL) then
           CALL ERRMSG2 (911, 0, '  Rdrain', STRING, IOUT1)
           Retval = 911
           Return
        Endif
...
!
  991 CONTINUE
      CALL ERRMSG2 (911, IECODE, 'Rdrain', ' neerslag file', IOUT1)
      RetVal = 911
      Return
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdrain', ' neerslag file', IOUT1)
      RetVal = 902
