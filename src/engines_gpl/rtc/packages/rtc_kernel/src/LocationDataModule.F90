...
!     was originally not initialized
      if (.not. success) then
        CALL ERRMSG2 (946, 0, 'ALLOC3DBAR', 'Allocation error 3DBarrier arrays', outuni)
        RetVal = 946
      endif
...
! RR data
      ...
      If (.not. success) then
         CALL ERRMSG2 (929, 1, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
...
! Sobek CF data
      ...
      If (.not. success) then
         CALL ERRMSG2 (929, 2, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
!    ID_SBF = ' '
...
! Precipitation data
      ...
      If (.not. success) then
         CALL ERRMSG2 (929, 3, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
...
! External/wind data
      ...
      If (.not. success)  then
         CALL ERRMSG2 (929, 4, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
      ...
      If (.not. success)  then
         CALL ERRMSG2 (929, 5, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
...
! WQ data
      ...
      If (.not. success)  then
         CALL ERRMSG2 (929, 6, ' AllocLocationArrays', ' ', IOUT1)
         RetVal = 929
         Return
      Endif
...
!         Read data using ParseToken
          ...
          If (Endfil .or. .not. success) then
             CALL ERRMSG2 (974, 0, 'Rdl_Sb', ' Unexpected end of Sobek locations file ',IOUT1)
             RetVal = 974
             Return
          Endif
          String = ' '
...
! check dimensies
          ...
          IF (ISBK .GT. NSBK) then
             CALL ERRMSG2 (913, 0, 'Rdl_sb',' NSBK Sobek lokaties',IOUT1)
             RetVal = 913
             Return
          Endif
! Read data
...
!             No branch, node, reachseg or struct found
              CALL ERRMSG2 (912, 0, ' bi in ir is ml hl bd',' Read Sobek lokaties',IOUT1)
              RetVal = 912
              Return
          Endif
...
! additional error handling
  ...
  992     Continue
          IF (ReadError) then
             CALL ERRMSG2 (974,0,'Rdl_Sb',' Read error during reading SBKO location record',IOUT1)
             RetVal = 974
             Return
          Endif
          ID_SBK (ISBK) = ID
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_sb', ' Sobek_id file', IOUT1)
      RetVal = 902
...
!          Read data using ParseToken
           ...
           Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
           If (Endfil .or. .not. success) then
             CALL ERRMSG2 (974, 0, 'Rdl_3B', ' Unexpected end of RR locations file ',IOUT1)
             RetVal = 974
             Return
           Endif
           String = ' '
...
! check dimensies
           IF (ID3B .GT. ND3B) then
             CALL ERRMSG2 (913, 0, 'Rdl_3b',' ND3B 3B-lokaties',IOUT1)
             RetVal = 913
             Return
           Endif
! end check
...
! get optional name
           ...
           IF (ReadError) then
             CALL ERRMSG2 (974,0,'Rdl_3B',' Read error during reading 3BO location record',IOUT1)
             RetVal = 974
             Return
           Endif
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_3B', ' Delft3B_id file', IOUT1)
      RetVal = 902
...
!         Read data using ParseToken
          ...
          Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
          If (Endfil .or. .not. success) then
             CALL ERRMSG2 (974, 0, 'Rdl_3D', ' Unexpected end of Sobek locations file ',IOUT1)
             RetVal = 974
             Return
          Endif
          String = ' '
...
! check dimensies
          ID3D = ID3D + 1
          IF (ID3D .GT. N3D) then
             CALL ERRMSG2 (913, 0, 'Rdl_3D',' N3D D3DFlow locations',IOUT1)
             RetVal = 913
             Return
          Endif
! Read data
...
! additional error handling
  ...
  992     Continue
          IF (ReadError) then
             CALL ERRMSG2 (974,0,'Rdl_3D',' Read error during reading D3DO location record',IOUT1)
             RetVal = 974
             Return
          Endif
          ID_D3D (ID3D) = ID
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_3D', ' D3DFlow_id file', IOUT1)
      RetVal = 902
...
!     IF (ENDFIL) CALL ERRMSG2 (911, 0, 'Rdl_PR', ' Precipitation id file', IOUT1)
!
! *********************************************************************
! *** read data
...
!          Read data using ParseToken
           ...
           Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
           If (Endfil .or. .not. success) then
             CALL ERRMSG2 (974, 0, 'Rdl_Pre', ' Unexpected end of Precipitations locations file ',IOUT1)
             RetVal = 974
             Return
           Endif
           String = ' '
...
! check dimensies
           IF (IPRECP .GT. NPRE) then
             CALL ERRMSG2 (913, 0,'Rdl_pr',' NPRE neerslaglokaties',IOUT1)
             RetVal = 913
             Return
           Endif
! end check
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_pr', ' neerslag_id file', IOUT1)
      RetVal = 902
...
!     IF (ENDFIL) CALL ERRMSG2 (911, 0, 'Rdl_Ex', ' External id file', IOUT1)
!
! *********************************************************************
...
!          Read data using ParseToken
           ...
           Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
           If (Endfil .or. .not. success) then
             CALL ERRMSG2 (974, 0, 'Rdl_Ext', ' Unexpected end of External locations file ',IOUT1)
             RetVal = 974
             Return
           Endif
           String = ' '
...
! check dimensies
           IF (IEXT .GT. NEXT) then
              CALL ERRMSG2 (913, 0,'Rdl_ex',' NEXT externe lokaties',IOUT1)
              RetVal = 913
              Return
           Endif
! end check
...
! check dimensies
           IF (IEXT .GT. NEXTH) then
              CALL ERRMSG2 (913, 0,'Rdl_ex',' NEXTH externe lokaties His file',IOUT1)
              RetVal = 913
              Return
           Endif
           IF (IDEBUG .GT. 0) WRITE(IDEBUG,*)  STRING(1:len_trim(String))
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_ex', ' external_id file', IOUT1)
      RetVal = 902
...
! check dimensies
           ...
           IF (ISobWQ .GT. NSWQ)  then
              CALL ERRMSG2 (913, 0, 'Rdl_wq',' NSWQ Sobek-WQ lokaties',IOUT1)
              RetVal = 913
              Return
           Endif
! Read data
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdl_wq', ' Sobek_wq id file', IOUT1)
      RetVal = 902
...
!      write(Idebug,*) ' Entry of CheckIdsMatlab', IdString(1:)
         ...
         if (CntStr('=',IdString) .ge. 2) then
            CALL ERRMSG2 (999, 1, ' CheckIdsMatlab', ' =sign in ids will not work when using Matlab', IOUT1)
            RetVal = 999
