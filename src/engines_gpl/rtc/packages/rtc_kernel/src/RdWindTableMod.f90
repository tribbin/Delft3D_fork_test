...
!
      ...
      If (ENDFIL) then
         CALL ERRMSG2 (911, 0, 'RdWindTableMod', ' Constant wind file', IOUT1)
         RetVal = 911
         Return
      Endif
!
...
!     Write(*,*)  ' RdWindTableMod Time:', RTC_TIMNEW, TIMEND
  ...
  102    Continue
         ...
         If (IWIND .GT. NWIND .or. .not. success) Then
            CALL ERRMSG2 (913, 0,'RdWindTableMod',' wind data lokaties',IOUT1)
            RetVal = 913
            Return
         EndIf
...
!     Get wind velocity data
         ...
         If (ILeftBuf .le. 0 .or. .not. success) then
            CALL ERRMSG2 (913, 0,'RdWindTableMod',' Error finding wv tv string',IOUT1)
            RetVal = 913
            Return
         Endif
...
! variabele wind velocity
            ...
            If (.not. success) then
               CALL ERRMSG2 (913, 0,'RdWindTableMod',' Error getting table name',IOUT1)
               RetVal = 913
               Return
            Endif
...
!       usually wv tv is before wd td; in that case use wv tv up to wd td, else from wv tv to end
               ...
               If (.not. success) then
                  CALL ERRMSG2 (913, 0,'RdWindTableMod',' Error getting table data',IOUT1)
                  RetVal = 913
                  Return
               Endif
!      Set references
...
!              Try to reduce table length
               ...
               If (.not. success) then
                  CALL ERRMSG2 (913, 0,'RdWindTableMod',' Error Reducing Table ',IOUT1)
                  RetVal = 913
                  Return
               Endif
            Endif
...
! ** wind direction
         ...
         If (ILeftBuf .le. 0) Then
            CALL ERRMSG2 (913, 0,'RdWindTableMod',' Error finding wd td string',IOUT1)
            RetVal = 913
            Return
         Endif
         Read(String(IleftBuf+6:),*,Err=150) Rdum(1), Rdum(2)
...
!
  150 Continue
      CALL ERRMSG2 (902, IECODE, 'RdWindTableMod', ' wind data file', IOUT1)
      RetVal = 902
