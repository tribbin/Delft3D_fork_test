...
!
      ...
      If (ENDFIL) then
         CALL ERRMSG2 (911, 0, 'ScanWind', ' Constant wind file', IOUT1)
         RetVal = 911
         Return
      Endif
!
...
! check dimensies
         If (IWIND .GT. NWIND) Then
            CALL ERRMSG2 (913, 0,'ScanWind',' wind data lokaties',IOUT1)
            RetVal = 913
            Return
         EndIf
! end check
...
!
  150 Continue
      CALL ERRMSG2 (902, IECODE, 'ScanWind', ' wind data file', IOUT1)
      RetVal = 902
...
!
  999 Continue
      ...
      Endif
      Do IExt = 1,NExtd
         if (WindLoc2Stat(iext) .eq. -1) CALL ERRMSG2 (942, 0, ' ScanWind', Id_ext(iext), IOUT1)
      Enddo
