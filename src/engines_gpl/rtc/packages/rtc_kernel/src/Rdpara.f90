...
!
      ...
      IF (ENDFIL) goto 21
!    *  CALL ERRMSG2 (911, 0, 'Rdpara', ' Beslisparameterfile', IOUT1)
!
! *********************************************************************
! *** read data
...
! Allocate and read interpolation tables
      If (NrInterpTables .gt. 0) then
         ...
         If (.not. success)  then
            CALL ERRMSG2 (929, 1, ' RdPara-interpolation tables', ' ', IOUT1)
            RetVal = 929
            Return
         Endif
         ITable = 0
         DO 10 IRecord =1,99999
            ...
            If (Endfil .or. .not. success)  then
               CALL ERRMSG2 (974, 0, 'Rdpara', ' Unexpected end of file ',IOUT1)
               RetVal = 974
               Return
            Else
               Success = GetTableName (TabYesNo, TableName, ' id ', Iout1)
               If (.not. success) then
                 CALL ERRMSG2 (913,0,'Rdpara',' Error getting Table Name',IOUT1)
                 RetVal = 913
                 Return
               Endif
               success = GetStringFromBuffer(String)
               IF (.not. success) then
                  CALL ERRMSG2 (913,0,'Rdpara',' Error getting String from Buffer',IOUT1)
                  RetVal = 913
                  Return
               Endif
               ITable = ITable + 1
...
!       check max. number of dec.variabelen
         IF (IPARA .gt. NDECV) then
            CALL ERRMSG2 (974,0,'Rdpara',' NDECV in decispar.rtc',IOUT1)
            RetVal = 974
            Return
         Endif
...
! check that id is not already used as decision parameter id
         DO JPARA=1,IPARA
            IF (PARAID(JPARA) .EQ. ID) THEN
               CALL ERRMSG2 (927,0, ID, ' Decision parameter file', IOUT1)
               RetVal = 927
               Return
            ENDIF
         ENDDO
...
!            If (Par2Use)  then
!               CALL ERRMSG2 (926, 0, 'Rdpara', ' ',IOUT1)
!               RetVal = 926
!               Return
!            Endif
             PARTYP (IPARA) = 'PARA'
...
! use Getrecord and ParseToken
         ...
         If (.not. Success) ReadError = .true.
         IF (ReadError) then
            CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading decision parameter file - PARA-PAR2 ParseToken ',IOUT1)
            RetVal = 974
            Return
         Endif
         If (Idebug .gt. 0) then
...
!           no proper value set for do keyword
         ...
         IF (ReadError) then
            CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading PAR2 decision parameter record',IOUT1)
            RetVal = 974
         Endif
         If (RetVal .ne. 0) Return
...
! Find the number of DATA subrecords within the String
         ...
         If (NrDataSubRecords .ne. NrSobekSubRecords + NrRRSubRecords + NrPrecipSubRecords + NrExtSubRecords + &
            ...
            CALL ERRMSG2 (974,0,'Rdpara',' PAR2 record contains inconsistent subrecords ',IOUT1)
            RetVal = 974
            Return
         Endif
         ALLOW = .FALSE.
...
! *********************************************************************
      IF (PARDIM(IPARA,1) .GT. NSPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' NSPAR Sobek lokaties', IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,2) .GT. N3PAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' N3PAR 3B lokaties', IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,3) .GT. NPPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' NPPAR neerslag lokaties',IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,4) .GT. NEPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' NEPAR externe lokaties',IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,5) .GT. NQPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' NQPAR beslisparameters',IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,6) .GT. NDPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' NDPAR beslisparameters',IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,8) .GT. 1) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' Interpolation references',IOUT1)
        RetVal = 913
      ELSEIF (PARDIM(IPARA,9) .GT. N3DPAR) THEN
        CALL ERRMSG2 (913, 0, 'Rdpara', ' D3DFlow locations',IOUT1)
        RetVal = 913
      ENDIF
      If (RetVal .ne. 0) Return
...
!        read error handling
   ...
   992   continue
         IF (ReadError) then
            CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading decision parameter record',IOUT1)
            RetVal = 974
            Return
         Endif
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,1)
           ...
           IF (IXSBHIS(IPARA,I) .LE. 0) THEN
               ...
               CALL ERRMSG2 (917,0, PARSBK(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,1)
            IF (TISSBK(IPARA,I) .LT. NTIMSH) THEN
                ...
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (915,NTIMSH, 'Rdpara', ' Beslisparameterfile', IOUT1)  ! is only a warning, so no return value
            ENDIF
            IF (TISSBK(IPARA,I) .GT. 0) THEN
                TISSBK(IPARA,I) = 0
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (923,0, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 923
            ENDIF
            If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,1)
           IF (VARIDSBK(IPARA,I) .eq. '') Then
             IF (VARSBK(IPARA,I) .GT. NPARS .or. VarSbk(Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPARS, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
             ENDIF
             If (RetVal .ne. 0) Return
           ENDIF
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,2)
           ...
           IF (IX3BHIS(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PAR3B(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,2)
            IF (TIS3B (IPARA,I) .LT. NTIMSH) THEN
                ...
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (915,NTIMSH, 'Rdpara', ' Beslisparameterfile', IOUT1)
            ENDIF
         ...
         DO I=1,PARDIM(IPARA,2)
           IF (VARID3B(IPARA,I) .eq. '') Then
             IF (VAR3B (IPARA,I) .GT. NPAR3 .or. Var3B(Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPAR3, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
             ENDIF
             If (RetVal .ne. 0) Return
           ENDIF
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,3)
           ...
           IF (IXPRHIS(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PARPRE(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,3)
            IF (VARPRE(IPARA,I) .GT. NPARP .or. VarPre(Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPARP, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
            ENDIF
            IF (VARPRE(IPARA,I) .GT. NTIMHP) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (939, NTIMHP, 'Rdpara',ParaId(ipara), IOUT1)
                RetVal = 939
            ENDIF
            If (RetVal .ne. 0) Return
         ENDDO
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,4)
           ...
           IF (IXEXHIS(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PAREXT(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,4)
            IF (VAREXT(IPARA,I) .GT. NPARE .or. VarExt(Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPARE, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
            ENDIF
            IF ((VAREXT(IPARA,I)+1)/2 .GT. NTIMHW) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (940, NTIMHW, 'Rdpara',ParaId(ipara), IOUT1)
                RetVal = 940
            ENDIF
            If (RetVal .ne. 0) Return
         ENDDO
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,5)
           ...
           IF (IXWQHIS(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PARWQ(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,5)
           IF (VARIDWQ(IPARA,I) .eq. '') Then
             IF (VARWQ (IPARA,I) .GT. NPARQ .or. VarWQ (Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPARQ, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
             ENDIF
             If (RetVal .ne. 0) Return
           ENDIF
...
! *********************************************************************
         ...
         DO I=1,PARDIM(IPARA,9)
           ...
           IF (IX3DHIS(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PAR3D(IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
         ...
         DO I=1,PARDIM(IPARA,9)
           IF (VARID3D(IPARA,I) .eq. '') Then
             IF (VAR3D (IPARA,I) .GT. NPAR3D .or. Var3D(Ipara,i) .le. 0) THEN
                WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                CALL ERRMSG2 (916, NPAR3D, 'Rdpara', ' Beslisparameterfile', IOUT1)
                RetVal = 916
             ENDIF
             If (RetVal .ne. 0) Return
           ENDIF
         ...
         If (.not. SearchPar2) Then
           DO I=1,PARDIM(IPARA,6)
              ...
              IF (TISPAR(IPARA,I) .GE. 0) THEN
                  TISPAR(IPARA,I) = -1
                  WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
                  CALL ERRMSG2 (923, -1, 'Rdpara', ' Beslisparameterfile', IOUT1)
                  RetVal = 923
              ENDIF
              If (RetVal .ne. 0) Return
...
! *********************************************************************
         DO I=1,PARDIM(IPARA,8)
           ...
           IF (IXInterpTable(IPARA,I) .LE. 0) THEN
               ...
               WRITE(IOUT1,*) ' Decision parameter ', PARAID(IPARA)
               CALL ERRMSG2 (917,0, PARInterp (IPARA,I), ' Beslisparameterfile', IOUT1)
               RetVal = 917
           ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
...
! test using GetRecord and ParseToken
         ...
         If (.not. Success) ReadError = .true.
         IF (ReadError) then
            CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading decision parameter file - RSVP ParseToken ',IOUT1)
            RetVal = 974
            Return
         Endif
         If (Idebug .gt. 0) then
...
! viz. max.lvl, initial level, expected inflow; downstream demand per link outlet, maximum flow per link outlet
         ...
         IF (PARDIM(IPARA,7) .GT. NRPAR) then
            CALL ERRMSG2 (913,0,'Rdpara',' NRPAR beslisparameters',IOUT1)
            RetVal = 913
         ElseIF (PARDIM(IPARA,7) .GT. NHLP)  then
            CALL ERRMSG2 (913,0,'Rdpara',' NHLP; PARDIM7',IOUT1)
            RetVal = 913
         Endif
         If (RetVal .ne. 0) Return
           ...
           if (Getkey ('gb', IStart, RecordData, NumberOfTokens, ReturnIndx, ParseTokenSearchCaseSensitive)) then
              Do I=1,NrBottomGates(Irsv)
                 ...
                 If (LinkOutlets(irsv,i,1) .gt. NrOutletLinks(irsv)) then
                    CALL ERRMSG2 (913,0,'Rdpara',' Inconsistent outlet link number specified for bottom gate ',IOUT1)
                    RetVal = 913
                    Return
                 Endif
              Enddo
           ...
           if (Getkey ('gt', IStart, RecordData, NumberOfTokens, ReturnIndx, ParseTokenSearchCaseSensitive)) then
              Do I=1,NrTurbines(Irsv)
                 ...
                 If (LinkOutlets(irsv,2,i) .gt. NrOutletLinks(irsv)) then
                    CALL ERRMSG2 (913,0,'Rdpara',' Inconsistent outlet link number specified for turbinegate ',IOUT1)
                    RetVal = 913
                    Return
                 Endif
              Enddo
           ...
           if (Getkey ('gs', IStart, RecordData, NumberOfTokens, ReturnIndx, ParseTokenSearchCaseSensitive)) then
              Do I=1,NrSpillways(Irsv)
                 ...
                 If (LinkOutlets(irsv,3,i) .gt. NrOutletLinks(irsv)) then
                    CALL ERRMSG2 (913,0,'Rdpara',' Inconsistent outlet link number specified for spillwaygate ',IOUT1)
                    RetVal = 913
                    Return
                 Endif
              Enddo
...
!        read error handling
   ...
   994   continue
         IF (ReadError) then
            CALL ERRMSG2 (974,0,'Rdpara',' Read error during reading Reservoir decision parameter record',IOUT1)
            RetVal = 974
            Return
         Endif
...
!        Get table, id=ParaId(ipara)
         ...
         If (Endfil .or. .not. success) then
            CALL ERRMSG2 (913,0,'Rdpara',' Error in PAR3 record: Time Table end not found',IOUT1)
            RetVal = 913
            Return
         else
            Success = GetTableName (TabYesNo, TableName, ' id ', Iout1)
...
!              Er is een tabel record PAR3 gedefinieerd, met een niet-lege naam
               ...
               If (.not. success) then
                  CALL ERRMSG2 (913,0,'Rdpara',' Error getting Table Data' ,IOUT1)
                  RetVal = 913
                  Return
               Endif
            Endif
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdpara', ' Beslispar.file', IOUT1)
      RetVal = 902
      Return
  ...
  253              Continue
         ...
         Else
            If (NSeq .lt. NPara) then
               CALL ERRMSG2 (951,0,'Rdpara',' No correct simulation order of decision variables can be set',IOUT1)
               RetVal = 951
               Return
            Endif
         Endif
...
! parameter id moet bekend zijn, dus IxdPar > 0
               ...
               CALL ERRMSG2 (924,0, PARAID(IPARA), ' Check 6 Decision parameter file', IOUT1)
               RetVal = 924
...
! parameters of van zichzelf, dus IxdPar <= IPara
               ...
               CALL ERRMSG2 (925,0, PARAID(IPARA), ' Decision parameter file', IOUT1)
               RetVal = 925
            ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
...
! parameter id moet bekend zijn, dus IxdPar > 0
               ...
               CALL ERRMSG2 (924,0, PARAID(IPARA), ' Check 7 Decision parameter file', IOUT1)
               RetVal = 924
            ENDIF
         ENDDO
         If (RetVal .ne. 0) Return
...
!     If the order in the input file is used, no inversion of Simseq is needed, since then SimSeq(i)=i for all i
      If (SetSequenceDecisionParameters) then
         ...
         If (Allocation_Error .ne. 0) then
            CALL ERRMSG2 (913,0,'Rdpara',' Error allocating arrays in subroutine',IOUT1)
            RetVal = 913
            Return
         Endif
         Do IPara=1,NPara
      ...
      Do i=1,9  ! was 6
         If (Abs(NTims) .lt. (2+Abs(MxTmShift(i))) ) then
            ...
            CALL ERRMSG2 (913, 0, 'Rdpara', ' Mx. Time shift dimension NTIMS', IOUT1)
            RetVal = 913
         Endif
      Enddo
!     D3DFlow
