...
!
      ...
      If (ENDFIL)  CALL ERRMSG2 (911, 0, 'Rdini', ' RTC-INIfile', IOUT1)
!
! *********************************************************************
! *** Initialisatie
! *********************************************************************
...
!          If (CDUM(1:LEN) .EQ. 'ASC' .OR. CDUM(1:LEN) .EQ. 'asc') IOPTWQ = 1
        ElseIf (STRING(1:POS1-1) .EQ. 'MODEPRECIPITATION') Then
           ...
           If (IMODEP .NE. 0) Then
              ...
              CALL ERRMSG2 (999, 0, 'Rdini', ' Option Mode<>Actual not yet supported', IOUT1)
!              STOP 999
           Endif
...
!             CALL ERRMSG2 (913, 0, 'Rdini', ' Time horizon', IOUT1)
!          Endif
  ...
  109 Read(IN,'(A)',END=21,ERR=150,IOSTAT=IECODE)  STRING
      ...
      If (POS1 .GT. 0) Then
        If (STRING(1:POS1-1) .EQ. 'DLLNAME') Then
           ...
           if (istat /= 0) then
              CALL ERRMSG2 (956, 0, 'Rdini', dllname(1:len_trim(dllname)), IOUT1)
           endif
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rdini', ' RTC_INIfile', IOUT1)
...
!
   21 CONTINUE
      ...
      If (dll_handle .ne. 0 .and. dll_function .eq. '') then
         CALL ERRMSG2 (957, 0, 'Rdini', ' open shared library function not defined ', iout1)
      endif
   ...
   202   Continue
         Do iDum=1,MatlabNrWqPar
            If (MatlabWqParId(idum) .eq. '') then
                CALL ERRMSG2 (952, Idum, 'Rdini', ' ', IOUT1)
            Endif
         EndDo
      Else
         MatWq = .false.
