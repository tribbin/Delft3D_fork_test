...
!*** type declarations
  ...
  interface GetRecord
    module procedure GetRecordKeyUppBeforeColumn
...
private NCalcTime
private LeapYear
private NewTable_ErrMsgScreen
private NewTable_ErrMsgFile
...
function NewTablesModelAdd(modelHandle) result(model)
    ...
    if ( SobekTimeTables % numModels >= MaxNumModels ) then
        write (*, '(A,I4,A)')  'ERROR: modelCreate: max #models (', MaxNumModels, ') exceeded'
    else
...
!         Allocate     ( TableData(MaxData), Stat=Allocation_Error )
         ...
         If (.not. success) then
            Call NewTable_ErrmsgScreen (981, Allocation_Error, ' Error allocating arrays in subroutine ', ' NewTable_ConfAr'   )
            NewTableConfAr = .false.
            Return
         Endif
...
!        write(*,*) ' MaxData  = ', MaxData
      ...
      If (Len(String) .lt. nbuf) then
        Call NewTable_ErrmsgScreen (981, 0, ' Stringsize too small to copy buffer contents ', ' NewTable_GetStringFromBuffer'  )
      ...
      Logical Function GetRecordKeyUppBeforeColumn ( lu , keywrd, Endfil, Idebug, Iout1, KeyUppOption)
      ! KeyUpp should occur before position KeyUppOption of the lines in the input file
      logical endfil
      integer lu, idebug, Iout1
      character(Len=MaxKeyLength)   keywrd
      integer     KeyUppOption

      GetRecordKeyUppBeforeColumn = GetRecordGeneral ( lu , keywrd, Endfil, Idebug, Iout1, KeyUppOption)

      Return
      End Function GetRecordKeyUppBeforeColumn
...
!              Eerst de ; en : verwijderen om dan free format te kunnen lezen en de periode in seconden te bepalen
            ...
            if (PeriodCode .ne. 31536000 .and. PeriodCode .ne. 30*86400 .and. PeriodCode .ne. 86400) then
               write(Iout1,*) ' Check Input Table ', TableName(1:32),' Specified value for Period is unlikely'
               write(Iout1,*) ' Likely values are 31536000 (year), 30*86400 (month) and 86400 (daily period)'
            endif
         endif
      Endif
...
!     NB LocalRows = aantal < tekens + 1, ivm mogelijke extra rij bij cyclische tabel
      ...
      If (Allocation_Error .ne. 0) then
        Call NewTable_ErrmsgFile (981, Allocation_Error, ' Error allocating arrays in subroutine ', ' NewTable_GetTable', Iout1  )
        GetTableFromBuffer = .false.
        Return
      Endif
...
!                   stop ' LocalRows dimension problem'
                    Return
                 endif
                 ileft  =  idum2+10
...
! May 2002: check NrColumns
                 ...
                 If (NumberOfTokens .lt. NrColumns+3) then
                    Call NewTable_ErrMsgFile (977, NrColumns, &
                        ' Table contains less columns then expected; will read less columns for table :', TableName, Iout1)
                    NrColumns = NumberOfTokens-3
                    If (NrColumns .le. 0) then
                       Call NewTable_ErrMsgFile (981, NrColumns, ' Error  getting Table ', TableName , Iout1)
                       GetTableFromBuffer = .false.
                       Return
                    Endif
                 Endif
...
! End ARS
              ...
              Else
                 Write(Iout1,*) ' Tables too long'
                 Write(Iout1,*) ' MaxData LocalNrRows NrColumns ', model%MaxData, LocalNrRows, NrColumns
                 Write(Iout1,*) ' FirstFreeDateTime Data        ', model%FirstFreeDateTime, model%FirstFreeData
                 Write(Iout1,*) ' Error NewTable dimensions'
                 Return
!                 Stop
              Endif
            Endif
         Else
            Write(Iout1,*) ' Too many tables'
            Write(Iout1,*) ' Error NewTable dimensions'
            Return
!            Stop
         Endif
...
!        Bij cyclische tabel, interpolatie: geef waarschuwing
         If (model%Ntab(TableNr,IndexInterpolationCode) .eq. 0 .and. PeriodError) then
            Write(iout1,*) ' Warning: Period specified for table not consistent with length of table', TableName(1:32)
...
!  Als RowNr>0: dan is rijnummer voor kolom 1 gegeven; bij columnr <> 1 alleen index ophogen
      ...
      If (TabelNr .gt. model%MaxTabNr) then
          ProgramError = GenerateMessage ( 3 , 'Table Number too large', ' ',' ' , Iout1)
          Stop ' Error: GetNewValue called with table nr which is too high'
      Endif
...
!     RowNr geeft nu het rijnummer in het Date/Time array
      ...
      If (i .gt. model%MaxData) then
         ...
         Stop ' GetNewValue called with table index too high'
      Endif
...
!     lineaire interpolatie tussen data waarde i en i+1, op basis van CurrentDate/Time en Julian dates
      ...
      If (i1 .gt. model%MaxData) then
         ProgramError = GenerateMessage ( 3 , 'Table Data index too large' , '   ', ' ' , Iout1)
         Stop ' Error: GetNewValue called with table data index which is too high'
      Endif
...
!     if (doorgaan) then
!     else
!        Stop ' Internal Search Error Periodic Table'
!     Endif
...
! Put result in Table arrays; table number = TableNr
      ...
      If (Allocation_Error .ne. 0) then
        Call NewTable_ErrmsgScreen (981, Allocation_Error, ' Error allocating arrays in subroutine ', &
                                            ' NewTable_MakeConstTable'  )
        MakeConstTable = .false.
        Return
      Endif
...
!***********************************************************************
      ...
      if (errtyp .eq. 1) then
        write (iout1, 1001) messag
      elseif (errtyp .eq. 2) then
        write ( iout1 , 1000 ) messag
      elseif (errtyp .eq. 3) then
        stopflag = .true.
        write ( * , 1110 ) messag
        write ( iout1 , 1100 ) messag
      else
      endif

      if (id  .ne. ' ') write (iout1, 1200) id
      if (nm  .ne. ' ') write (iout1, 1300) nm

      if (stopflag) then
          close (iout1)
!         STOP ' NewTable error, run stopped'
      endif
 ...
 1001 format (/ 'Information: ',A)
 1100 format (/ 'Error: ',A)
 1110 format (/' Error: ',A)
 1200 format (  'Object-id  : ',A)
 1300 format (  'Object-name: ',A)
...
! Functie bepaalt of jaar IYEAR een schrikkeljaar is
  ...
  SUBROUTINE NewTable_ErrMsgFile (ICODE, IECODE, STR1, STR2, Iout1)
    ! *********************************************************************
    ! *** DELFT_3B model
...
!     STRING(3) = ' ErrorCode ' // INTCH8(IECode)
      ...
      Call WriteMessage (Iout1, Messg)
      Return

  Return
  End Subroutine NewTable_ErrMsgFile


  SUBROUTINE NewTable_ErrMsgScreen (ICODE, IECODE, STR1, STR2)
    ! *********************************************************************
    ! *** DELFT_3B model
    ! *** LAST UPDATE : January 21, 1997          BY : Peter Schrier
    ! ***   conversion of dutch captions to english captions
...
!     STRING(3) = ' ErrorCode ' // INTCH8(IECode)
  ...
  Return
  End Subroutine NewTable_ErrMsgScreen
