...
!
      ...
      If (ENDFIL) then
         CALL ERRMSG2 (911, 0, 'RDWIND', ' Constant wind file', IOUT1)
         RetVal = 911
         Return
      Endif
!
...
! check dimensies
         If (IWIND .GT. NWIND) Then
            CALL ERRMSG2 (913, 0,'Rdwind',' wind data lokaties',IOUT1)
            RetVal = 913
            Return
         EndIf
! end check
...
!               If (IDEBUG .GT. 0) Write(IDEBUG,*)  ' Time:', TIMRD, RTC_TIMNEW, TIMEND
                If (TIMRD .GE. RTC_TIMNEW .AND. TIMRD .LE. TIMEND) then
                  ...
                  If (INDX .GT. NTIMW) Then
                     CALL ERRMSG2 (913, 0,'Rdwind',' wind tijdserie',IOUT1)
                     RetVal = 913
                     Return
                  EndIf
...
!
  150 Continue
      CALL ERRMSG2 (902, IECODE, 'Rdwind', ' wind data file', IOUT1)
      RetVal = 902
