...
! ARS 7623: check op array bounds
             if (lr+l .gt. 999) then
               Write(iout1,*) ' Error in concatenating strings'
...
!           WRITE(*,*) ' after call FNDFRST/GETSTR ', STRING(1:)
         ...
         If (ReadError .and. .not. allow) Then
              Write(Iout1,*) ' Error at reading from string', String(1:lengtestring)
...
!     CHARACTER*999 GETSTR
      ...
      IF (IPOS .GT. -1) THEN
         ...
         ENDIF
         If (ReadError .and. .not. allow) Then
              Write(Iout1,*) ' Error at reading from string',&
                               String(1:lengtestring)
              CALL ERRMSGLIB2(801, 0, FILSUB, SEARCH, IOUT1, IflRtn)
              RetVal = 801
              Return
         Endif
...
!              quote or doublequote: character
        ...
        ELSE
!          Unknown type ITYPE
           Write(Iout1,*) ' Error searching string ', SEARCH, &
                          ' with unknown type', IType
           CALL ERRMSGLIB2(801, 0, FILSUB, SEARCH, IOUT1, IflRtn)
           RetVal = 801
           Return
        ENDIF
