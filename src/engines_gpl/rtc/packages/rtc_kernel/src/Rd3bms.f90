...
!c    IF (ENDFIL)
!c   *  CALL ERRMSG2 (911, 0, 'Rd3bms', ' 3B-Maatregelfile', IOUT1)
...
! *********************************************************************
           ...
           If (Endfil .or. .not. success) then
              CALL ERRMSG2 (974, 0, 'Rd3bMs', ' Unexpected end of file ',IOUT1)
              RetVal = 974
              Return
           Endif
           String = ' '
           ...
           IF (ReadError) then
              ...
              CALL ERRMSG2 (974,0,'Rd3BMs',ErrorString, IOUT1)
              RetVal = 920
              Return
           Endif
...
! check On and Off checks '<', '=', '>'
           IF ( ONCH3B(N3MEAS) .NE. '<' .and. ONCH3B(N3MEAS) .NE. '=' .and.  ONCH3B(N3MEAS) .NE. '>' ) THEN
              CALL ERRMSG2 (920,N3MEAS,MSID3B(N3MEAS),ONCH3B(N3MEAS),IOUT1)
              RetVal = 920
              Return
           ENDIF
! off check
           ...
           IF ( OFCH3B(N3MEAS) .NE. '<' .and. OFCH3B(N3MEAS) .NE. '=' .and. OFCH3B(N3MEAS) .NE. '>' ) THEN
              CALL ERRMSG2 (920,N3MEAS,MSID3B(N3MEAS),OFCH3B(N3MEAS),IOUT1)
              RetVal = 920
              Return
           ENDIF
...
! *********************************************************************
           ...
           IF (IXMS3P(N3MEAS) .LE. 0) THEN
              ...
              CALL ERRMSG2 (919,0, MSBP3B(N3MEAS), ' 3B-measure file', IOUT1)
              RetVal = 919
              Return
           ENDIF
...
!
            ...
            IF (ReadError) then
               CALL ERRMSG2 (974,0,'Rd3BMs',' Read error during reading RTC measures RRST records for RR module',IOUT1)
               RetVal = 920
               Return
            Endif
...
! *********************************************************************
      ...
      IF (N3MEAS .GT. N3MES) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms',' N3MES 3B measures',IOUT1)
         RetVal = 913
         Return
      ELSEIF (N3MLOC+N3MatLoc .GT. N3LOC) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms',' N3LOC 3B lokatie-measures',IOUT1)
         RetVal = 913
         Return
      ELSEIF (N3MATLOC .GT. N3MAT) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms',' N3MATLOC 3B Matlab-measures',IOUT1)
         RetVal = 913
         Return
      ENDIF
      If (N3meas .eq. 0) then
...
! *********************************************************************
  ...
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rd3bms', ' 3B-Maatregelfile', IOUT1)
      RetVal = 902
      Return
...
!           WRITE(IOUT1,*) ' Measure',N3MEAS
            ...
            Enddo
            CALL ERRMSG2 (922,0, LCMS3B(I3LOC),  ' 3B-measure file', IOUT1)
            RetVal = 922
            Return
         ENDIF
      ENDDO
...
! *********************************************************************
      ...
      CALL SKPCOM (IN, ENDFIL,'RTC')
!c    IF (ENDFIL)
!c   *  CALL ERRMSG2 (911, 0, 'Rd3bms_oldformat', ' 3B-Maatregelfile', IOUT1)
...
! *********************************************************************
           ...
           Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
           If (Endfil .or. .not. success) then
              CALL ERRMSG2 (974, 0, 'Rd3bMs_oldformat', ' Unexpected end of file ',IOUT1)
              RetVal = 974
              Return
           Endif
           String = ' '
...
! off check
           ...
           ENDIF
           IF (ReadError) then
              CALL ERRMSG2 (974,0,'Rd3BMs_oldformat',' Read error during reading RTC measures MLST records for RR module',IOUT1)
              RetVal = 920
              Return
           Endif
...
! total number of gates
           ...
           IF (ReadError) then
              CALL ERRMSG2 (974,0,'Rd3bMs_oldformat',' Read error during reading RTC measure 3BML record for RR module',IOUT1)
              RetVal = 974
              Return
           ENDIF
!
...
! *********************************************************************
      ...
      IF (N3MEAS .GT. N3MES) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms_oldformat',' N3MES 3B measures',IOUT1)
         RetVal = 913
         Return
      ELSEIF (N3MLOC+N3MatLoc .GT. N3LOC) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms_oldformat',' N3LOC 3B lokatie-measures',IOUT1)
         RetVal = 913
         Return
      ELSEIF (N3MATLOC .GT. N3MAT) THEN
         CALL ERRMSG2 (913, 0,'Rd3bms_oldformat',' N3MATLOC 3B Matlab-measures',IOUT1)
         RetVal = 913
         Return
      ENDIF
! end check
...
! *********************************************************************
  ...
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE, 'Rd3bms_oldformat', ' 3B-Maatregelfile', IOUT1)
      RetVal = 902
      Return
