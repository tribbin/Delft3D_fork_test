...
!        write(Idebug,*) ' NHIS, NLOCHIS ', NHIS, NLOCHIS
        ...
        IF (NLOCF .GT. NLOCHIS) THEN
            CALL ERRMSG2 (913, 0, NAME,' NLOCaties in HIS file',IOUT1)
            RetVal = 913
        ELSEIF (NHISP .GT. NHIS) THEN
            CALL ERRMSG2 (913, 0, NAME,' NPARameters in HIS file',IOUT1)
            RetVal = 913
        ENDIF
        ...
        ! *********************************************************************
        ! Already done above
        ! IF (NHISP .GT. NHIS)  CALL ERRMSG2 (913, 0, NAME,' NPARameters in HIS file',IOUT1)
        ...
        If (ITmstp .eq. 0) then
            ...
            Do iloc=1,Nloc
               If (.not. FoundLoc(iloc)) then
                  ...
                  CALL ERRMSG2 (973, 0, ' ReadDioPlt ', MsgString, IOUT1)
               endif
            Enddo
        Endif
    Else
...
!    write(*,*) ' ReadDioPlt before DioPltGet'
    if (.not. DioPltGet(inDataSet, readValues)) then
       if (NLocf * NHisp .gt. 0) then
          ...
          write(*,*) MsgString
          if (len_trim(MsgString) .gt. 0) CALL ERRMSG2 (9131, 0, ' ReadDioPlt ', MsgString(1:len_trim(MsgString)), IOUT1)
          CALL ERRMSG2 (9131, 0, ' ReadDioPlt ',' DIO Could not get the values from RR/Flow/3DFlow ',IOUT1)
          RetVal = 913
       endif
    endif
    if (RetVal .ne. 0) Return
...
!
    if ( doHeader ) then
        ...
        If (NTimes .gt. NTimHis) then
           CALL ERRMSG2 (913, 0, 'Rdpara', ' NTimHis parameter', IOUT1)
           RetVal = 913
        endif

        Allocate  (IDPara (NHisP), IdRead(NLocF), STAT=Allocation_Error )
        If (Allocation_Error .ne. 0) then
            CALL ERRMSG2 (929, Allocation_Error, ' Alloc ExtLoc Arrays', ' ', IOUT1)
            RetVal = 929
        endif
        if (RetVal .ne. 0) Return
...
! *********************************************************************
    if (.not. DioPltGet(inDataSet, readValues)) then
       if (NLocf * NHisp .gt. 0) then
          write(*,*) 'Could not read values in RdHis'
          CALL ERRMSG2 (974, 0, ' ReadDioPltExt ',' DIO Could not get the values from RR ',IOUT1)
          RetVal = 974
          Return
       endif
    endif
...
!   Find number of actual locations and parameters to get from the dataset
    ...
    If (Allocation_Error .ne. 0) Then
        CALL ERRMSG2 (929, Allocation_Error, ' AllocLocationArrays', ' ', IOUT1)
        RetVal = 929
        Return
    Endif
    HisParIndex = 0
       ...
       If (HisDataSet(IExt,1) .eq. IExtDataSet) then
          ...
          if (HisDataSet(Iext,2) .eq. 0 .or. HisDataSet(Iext,3) .eq. 0) then
             ...
             if (HisDataSet(Iext,2) .eq. 0) then
                CALL ERRMSG2 (959, Allocation_Error, ' ReadDioPLtExtSelection ',  HisParExt(iext) , IOUT1)
                RetVal = 959
                Return
             endif
             if (HisDataSet(Iext,3) .eq. 0) then
                CALL ERRMSG2 (959, Allocation_Error, ' ReadDioPltExtSelection ',  HisLocExt(iext) , IOUT1)
                RetVal = 959
                Return
             endif
          endif
...
! end check missing values!! juli 2011
    ...
    Elseif (IPar .gt. 0) then
       ...
       CALL ERRMSG2 (974, 0, ' ReadDioPltExtSelection ',' DIO Could not get the values from Ext HIS file',IOUT1)
       RetVal = 974
       Return
    Endif
    DeAllocate  (Values)
...
!    write(*,*) ' WriteDio Outname=',OutName
    ...
    IF (outName .NE. ' ' .AND. N3bMs .GT. 0) THEN
        if ( doHeaderRR ) then
            ...
            If (.not. success) then
                CALL ERRMSG2 (981, 0, ' RTC', ' Error allocating arrays ', Iout1)
                RetVal = 981
                Return
            Endif
            ! header
