...
!
      IF (NHLP .LT. NSCV) then
         CALL ERRMSG2 (913, 0,'Rdsmeas',' NHLP local parameter', IOUT1)
         RetVal = 913
         Return
      Endif
!
...
! check dimensies
         IF (IMEAS .GT. NSMES) then
            CALL ERRMSG2 (913, 0,'Rdsmeas',' NSMES Sobek measures',IOUT1)
            RetVal = 913
            Return
         Endif
! end check
! Get FLCM record, controller id and notactive indication
         ...
         If (Endfil .or. .not. success) then
            CALL ERRMSG2 (913, 0, 'RdSMeas', ' Unexpected end of file ',IOUT1)
            RetVal = 913
            Return
         Endif
         String = ' '
         ...
         IF (ReadError) then
             ...
             CALL ERRMSG2 (974,0,'RdSBMs',ErrorString,IOUT1)
             RetVal = 974
             Return
         Endif
...
!        NotActive option: na 0 = active, na 1 = not active
         ...
         If (ReadError) then
              ErrorString = ' Some error in general part of FLCM record for RTC-measure for CF ' // &
                              String(1:len_trim(String))
              CALL ERRMSG2 (974,0,'RdSMeas',ErrorString,IOUT1)
              RetVal = 974
              Return
         endif
            ...
            IF (MEASTY(IMEAS) .EQ. 9 .or. MEASTY(Imeas) .eq. 11 .or. MeasTy(Imeas) .eq. 12) THEN
               ...
               elseif (MeasTy(imeas) .eq. 11) then
                  ...
                  if (dll_handle .eq. 0) then
                     CALL ERRMSG2 (958,IMEAS, ErrorString,' Warning: DLL measure defined, but no DLL shared library defined', IOUT1)
                  endif
               elseif (MeasTy(imeas) .eq. 12) then
                  if (Getkey ('ti', StartSubRecord, RecordData, EndSubRecord, ReturnIndx, ParseTokenSearchCaseSensitive)) then
                     ...
                     CALL ERRMSG2 (958,IMEAS, ErrorString,' Warning: measure type 12, but no measure id with keyword ti defined', IOUT1)
                  endif
! check that csv files are specified if type 12 measure used
                  if (.not. UseTCN) then
                     CALL ERRMSG2 (958,IMEAS, ErrorString,' Error: measure type 12 used, but UseTCN not switched on in Settings', IOUT1)
                  endif
                  if (WriteCsvFile .eq. '' .or. ReadCsvFile .eq. '' .or. RunCommand .eq. '') then
                     CALL ERRMSG2 (958,IMEAS, ErrorString,' Error: measure type 12 used, but no external command or csv files specified', IOUT1)
                  endif
               endif
...
!                 mi missing is allowed, then Measure Id is used in communicating with Matlab
                  if (MeasTy(imeas) .eq. 9) then
                    ...
                    CALL ERRMSG2 (994,IMEAS, ErrorString,' Warning: Matlab communication id is missing for this measure', IOUT1)
                  endif
               endif
...
! type 2: SBMS id 'Sobek-id' bp 'Beslispar-id' pr iprior ty 2 nv nval cv N_checkvals sp N_setpoints sbms
              ...
              IF (MEASNV(IMEAS) .GT. NHLP .OR. MEASNV(Imeas) .gt. NSCV)  then
                 CALL ERRMSG2 (913,0,'Rdsmeas',' NHLP; MEASNV',IOUT1)
                 RetVal = 913
                 Return
              Endif
              if (Getkey ('cv', IStart, RecordData, EndSubRecord, ReturnIndx, ParseTokenSearchCaseSensitive)) then
...
! all types of measures
            ...
            IF (IXMSBP(IMEAS) .LE. 0) THEN
              IF (MEASBP(IMEAS)(1:6) .NE. 'Matlab' .and. MEASBP(IMEAS)(1:3) .NE. 'Dll' .and. MeasBP(Imeas)(1:3) .ne. 'TCN') THEN
                 ...
                 ENDDO
                 CALL ERRMSG2 (919,0, MEASBP(IMEAS)(1:len_trim(MeasBP(Imeas))), ' Sobek-measure file', IOUT1)
                 RetVal = 919
                 Return
                ...
                IF (IXMSNBP(INR,IMEAS) .LE. 0) THEN
                  IF (MEASNBP(INR,IMEAS)(1:6) .NE. 'Matlab' .and. MEASNBP(INR,IMEAS)(1:3) .ne. 'Dll'.and. MEASNBP(INR,IMEAS)(1:3) .ne. 'TCN') THEN
                    ...
                    DO IPARA=1,NPARA
                      WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                    ENDDO
                    CALL ERRMSG2 (919,0, MEASNBP(INR,IMEAS)(1:len_trim(MeasNBP(Inr,Imeas))), ' Sobek-measure file', IOUT1)
                    RetVal = 919
                    Return
            ...
            IF (MEASTY(IMEAS) .EQ. 3 .OR. MEASTY(IMEAS) .EQ. 4) THEN
               ...
               IF (IXMSCP(IMEAS) .LE. 0) THEN
                  ...
                  DO IPARA=1,NPARA
                     WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                  ENDDO
                  CALL ERRMSG2 (919,0, MEASCP(IMEAS)(1:len_trim(MeasCP(Imeas))), ' Sobek-measure file', IOUT1)
                  RetVal = 919
                  Return
               ENDIF
            ENDIF
              ...
              DO INR=1,MEASNV(IMEAS)
                 ...
                 IF (IXMSNCP(INR,IMEAS) .LE. 0) THEN
                    ...
                    DO IPARA=1,NPARA
                       WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                    ENDDO
                    CALL ERRMSG2 (919,0, MEASNCP(INR,IMEAS)(1:len_trim(MeasNCP(Inr,Imeas))),' Sobek-measure file', IOUT1)
                    RetVal = 919
                    Return
                 ENDIF
              ENDDO
            ...
            IF (MEASTY(IMEAS) .EQ. 4 .OR. MEASTY(IMEAS) .EQ. 6 .OR.  &
               ...
               IF (IXMSSP(IMEAS) .LE. 0) THEN
                  ...
                  DO IPARA=1,NPARA
                     WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                  ENDDO
                  CALL ERRMSG2 (919,0, MEASCSP(IMEAS)(1:len_trim(MEASCSP(Imeas))), ' Sobek-measure file', IOUT1)
                  RetVal = 919
                  Return
               ENDIF
            ENDIF
...
! Check correct type of checks ('<', '=', '>')
            IF (MEASTY(IMEAS) .LE. 4. .and. MEASTY(IMEAS) .ne. 2) Then
               IF ( MEASCH(IMEAS) .NE. '<' .AND. MEASCH(IMEAS) .NE. '=' .AND. &
                 ...
                 ErrorString = ' Measure id = '// MeasId(imeas)(1:len_trim(MeasId(imeas))) //  &
                               ' Measure name = '// MeasName(imeas)(1:len_trim(MeasName(imeas)))
                 CALL ERRMSG2 (920,IMEAS, ErrorString, MEASCH(IMEAS), IOUT1)
                 RetVal = 920
                 Return
               ENDIF
            ELSEIF (MEASTY(IMEAS) .GE. 5. .and. MEASTY(IMEAS) .LE. 8) Then
               DO INR=1,MEASNV(IMEAS)
                 IF ( MEASNCH(INR,IMEAS) .NE. '<' .AND. MEASNCH(INR,IMEAS) .NE. '=' .AND. &
                   ...
                   ErrorString = ' Measure id = '// MeasId(imeas)(1:len_trim(MeasId(imeas))) //  &
                                 ' Measure name = '// MeasName(imeas)(1:len_trim(MeasName(imeas)))
                   CALL ERRMSG2 (920,IMEAS, ErrorString, MEASNCH(INR,IMEAS), IOUT1)
                   RetVal = 920
                   Return
                 ENDIF
               ENDDO
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE,'Rdsmeas',' Sbk-measurefile',IOUT1)
      RetVal = 902
      Return
...
!
      IF (NHLP .LT. NSCV) then
         CALL ERRMSG2 (913, 0,'Rdsmeas_OldFormat',' NHLP local parameter', IOUT1)
         RetVal = 913
         Return
      Endif
!
...
! check dimensies
         IF (IMEAS .GT. NSMES) then
            CALL ERRMSG2 (913, 0,'Rdsmeas_OldFormat',' NSMES Sobek measures',IOUT1)
            RetVal = 913
            Return
         Endif
! end check
         ...
         Success = GetRecord (In, SearchString, Endfil, Idebug, Iout1)
         If (Endfil .or. .not. success) then
            CALL ERRMSG2 (913, 0, 'Rdpara', ' Unexpected end of file ',IOUT1)
            RetVal = 913
            Return
         Endif
         String = ' '
...
! type 2: SBMS id 'Sobek-id' bp 'Beslispar-id' pr iprior ty 2 nv nval cv N_checkvals sp N_setpoints sbms
           ...
           IF (MEASNV(IMEAS) .GT. NHLP)  then
              CALL ERRMSG2 (913,0,'Rdsmeas_OldFormat',' NHLP; MEASNV',IOUT1)
              RetVal = 913
              Return
           Endif
           if (Getkey ('cv', IStart, RecordData, NumberOfTokens, ReturnIndx, ParseTokenSearchCaseSensitive)) then
...
! additional read error label
  ...
  992    Continue
         ...
         IF (ReadError) then
             CALL ERRMSG2 (974,0,'RdSBMs',' Read error during reading RTC measures SBMS records for CF module',IOUT1)
             RetVal = 974
             Return
         Endif
         ...
         IF (IXMSBP(IMEAS) .LE. 0) THEN
           IF (MEASBP(IMEAS)(1:6) .NE. 'Matlab' ) THEN
              ...
              DO IPARA=1,NPARA
                 WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
              ENDDO
              CALL ERRMSG2 (919,0, MEASBP(IMEAS), ' Sobek-measure file', IOUT1)
              RetVal = 919
              Return
           ELSE
             ...
             IF (IXMSNBP(INR,IMEAS) .LE. 0) THEN
               IF (MEASNBP(INR,IMEAS)(1:6) .NE. 'Matlab' ) THEN
                 ...
                 DO IPARA=1,NPARA
                   WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                 ENDDO
                 CALL ERRMSG2 (919,0, MEASNBP(INR,IMEAS), ' Sobek-measure file', IOUT1)
                 RetVal = 919
                 Return
               ELSE
!               Matlab maatregel: voeg beslisparameter toe
         ...
         IF (MEASTY(IMEAS) .EQ. 3 .OR. MEASTY(IMEAS) .EQ. 4) THEN
            ...
            IF (IXMSCP(IMEAS) .LE. 0) THEN
               ...
               DO IPARA=1,NPARA
                  WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
               ENDDO
               CALL ERRMSG2 (919,0, MEASCP(IMEAS), ' Sobek-measure file', IOUT1)
               RetVal = 919
               Return
            ENDIF
         ENDIF
           ...
           DO INR=1,MEASNV(IMEAS)
              ...
              IF (IXMSNCP(INR,IMEAS) .LE. 0) THEN
                 ...
                 DO IPARA=1,NPARA
                    WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
                 ENDDO
                 CALL ERRMSG2 (919,0, MEASNCP(INR,IMEAS),' Sobek-measure file', IOUT1)
                 RetVal = 919
                 Return
              ENDIF
           ENDDO
         ...
         IF (MEASTY(IMEAS) .EQ. 4 .OR. MEASTY(IMEAS) .EQ. 6 .OR.  MEASTY(IMEAS) .EQ. 8) THEN
            ...
            IF (IXMSSP(IMEAS) .LE. 0) THEN
               ...
               DO IPARA=1,NPARA
                  WRITE(IOUT1,*) ' Parameter id',PARAID(IPARA)(1:len_trim(ParaId(Ipara)))
               ENDDO
               CALL ERRMSG2 (919,0, MEASCSP(IMEAS), ' Sobek-measure file', IOUT1)
               RetVal = 919
               Return
            ENDIF
         ENDIF
...
! Check correct type of checks ('<', '=', '>')
         IF (MEASTY(IMEAS) .LE. 4. .and. MEASTY(IMEAS) .ne. 2) Then
            IF ( MEASCH(IMEAS) .NE. '<' .AND. MEASCH(IMEAS) .NE. '=' .AND. &
                                                  MEASCH(IMEAS) .NE. '>')   THEN
              CALL ERRMSG2 (920,IMEAS, MEASID(IMEAS), MEASCH(IMEAS), IOUT1)
              RetVal = 920
              Return
            ENDIF
         ELSEIF (MEASTY(IMEAS) .GE. 5. .and. MEASTY(IMEAS) .LE. 8) Then
            DO INR=1,MEASNV(IMEAS)
              IF ( MEASNCH(INR,IMEAS) .NE. '<' .AND. MEASNCH(INR,IMEAS) .NE. '=' .AND. &
                                                         MEASNCH(INR,IMEAS) .NE. '>')  THEN
                CALL ERRMSG2 (920,IMEAS, MEASID(IMEAS), MEASNCH(INR,IMEAS), IOUT1)
                RetVal = 920
                Return
              ENDIF
            ENDDO
...
!
  150 CONTINUE
      CALL ERRMSG2 (902, IECODE,'Rdsmeas_OldFormat',' Sbk-measurefile',IOUT1)
      RetVal = 902
      Return
