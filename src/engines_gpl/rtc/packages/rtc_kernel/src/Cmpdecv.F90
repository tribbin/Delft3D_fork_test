...
! *********************************************************************
       ...
       ElseIf (ParTyp(IPARA) .eq. 'RSVP') then
           ...
           If (RetVal .ne. 0) then
              CALL ERRMSG2 (RetVal, 0,' Reservoir simulation ',' ',IOUT1)
              Return
           Endif
...
!           write(*,*) ' first allocate arrays for dll communication'
            ...
            if (.not. success) then
               CALL ERRMSG2 (958, 0, 'RTC', ' Error allocating arrays for dll call', IOUT1)
            endif
...
!                                 NrRRDllmeasures, .., rrslowon, rrslowoff, rrshighon, rrshighoff, &
            if (returncode .ne. -999) then
                ...
                CALL ERRMSG2 (958, 0, ' Error calling dll', ' Error calling dll', IOUT1)
            else
                if (idebug .gt. 0) then
...
! voer operatie uit
      ...
      ELSEIF (PAROPER(IPARA) .EQ. 4) THEN
         IF (ABS(RHLP) .LE. .0000001) THEN
            STRING = PARAID(IPARA)
            CALL ERRMSG2 (936, 0, ' Cmpdecv', STRING, IOUT1)
...
!        do Interpolation, using DCVVal(Ipara,1) as input
      ...
      ELSE
         ...
         CALL ERRMSG2 (994, 0, String, ' Operation only allowed using 1 argument', IOUT1)
         RetVal = 994
      ENDIF
...
! voer operatie uit
      ...
      ELSEIF (PAROPER(IPARA) .EQ. 15) THEN
         If (abs(Rhlp) .gt. 1) Then
              ...
              CALL ERRMSG2 (994, 0, String, ' Argument outside allowed range', IOUT1)
              RetVal = 994
              Return
         Endif
      ...
      ELSE
         String = 'Compute decision parameter ' // (ParaId(Ipara)(1:len_trim(ParaId(ipara))))
         CALL ERRMSG2 (994, 0, String, ' unknown operation', IOUT1)
         RetVal = 994
      ENDIF
