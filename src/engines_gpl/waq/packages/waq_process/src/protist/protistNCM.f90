...
!
!     support variables
    integer(kind=int_32), parameter     ::nrIndInp = 5    !   nr of species independent input items
    integer(kind=int_32), parameter     ::nrSpecInp = 32  !   nr of inputs per species
    integer(kind=int_32), parameter     ::nrSpecOut = 35  !   nr of outputs per species
    integer(kind=int_32), parameter     ::nrSpecFlux = 23 !   nr of fluxes per species
    ...
    real(kind=sp)        ::relPhag                             ! feeding night:day
    real(kind=sp)        ::UmRT, Q10, RT, CR                   ! growth and respiration rate calculation
    real(kind=sp)        ::NCm, PCm                            ! maximum NC and PC quotas
    real(kind=sp)        ::NCo, PCo                            ! minimum NC and PC quotas
    real(kind=sp)        ::NCopt, PCopt                        ! optimal NC and PC quotas
    real(kind=sp)        ::ChlCm, degChl
    ...
    real(kind=sp)        ::redco, PSDOC, maxPSreq, relPS       ! photosynthesis related input
    real(kind=sp)        ::alpha                               ! inital slope

    ! INPUT STATE VARIABLES
    real(kind=sp)     ::protC, protN, protP, protChl           ! protist state variables
    ...
    real(kind=sp)     ::NC, PC, ChlC    ! nutrient quotas
    real(kind=sp)     ::UmT, BR         ! growth and repsiration rates
    real(kind=sp)     ::NCu, PCu, NPCu  ! nutrient limitations
    ...
    ! photosynthesis
    real(kind=sp)     ::PSqm, PS, Cfix

    ! other parameters
    real(kind=sp), parameter  ::wTurb = 0.0 ! this needs to be an input from model eventually!!!!

    ! Fluxes
    real(kind=sp)    ::dCeat, dNeat, dPeat                         ! assimilation fluxes
    real(kind=sp)    ::dCresp                                      ! respiration flux
    real(kind=sp)    ::dCfix, dChldeg, dChlout                     ! photosynthesis and Chl degradation fluxes
    real(kind=sp)    ::dDOCleak, dDOCvoid                          ! DOC leakage
    real(kind=sp)    ::dPOCout, dPONout, dPOPout                   ! voiding organic fluxes
    real(kind=sp)    ::dNH4out, dPout                              ! voding inorganic fluxes
    real(kind=sp)    ::dAutC, dAutN, dAutP, dAutChl                ! autolysis fluxes
    real(kind=sp)    ::dDetC, dDetN, dDetP, dDetChl                ! detritus fluxes
    real(kind=sp)    ::dChlup                                      ! uptake of prey chlorophyll
