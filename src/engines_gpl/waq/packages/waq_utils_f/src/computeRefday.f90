...
use m_waq_type_definitions
    ...
    contains
      ...
      subroutine compute_refday_from_string(refdat, refday)
        ...
        integer(kind=int_32)                       ::   iyear              !  year of the time offset
        integer(kind=int_32)                       ::   imonth             !  month of the time offset
        integer(kind=int_32)                       ::   iday               !  day of the time offset
        integer(kind=int_32)   , intent(out)       ::   refday             !  refday
      ...
      subroutine compute_refday_from_integers(iyear, imonth, iday, refday)
        IMPLICIT  NONE
        integer(kind=int_32)   , intent(in)  ::   iyear              !  year of the time offset
        integer(kind=int_32)   , intent(in)  ::   imonth             !  month of the time offset
        integer(kind=int_32)   , intent(in)  ::   iday               !  day of the time offset
        integer(kind=int_32)   , intent(out) ::   refday             !  refday
        
        integer(kind=int_32), dimension(12) ::daysPerMonth       !  # days in each month
