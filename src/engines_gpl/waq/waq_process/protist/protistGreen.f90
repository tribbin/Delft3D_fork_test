...
contains
    ...
    subroutine PROGRE     (process_space_real, fl, ipoint, increm, num_cells, &
        ...
        segmentLoop : do iseg = 1, num_cells
            ...
            if (ikmrk1==1) then
                ...
                Temp = process_space_real(ipnt(5))  !    ambient water temperature                              (oC)
                PFD = process_space_real(ipnt(6))  !    from rad to photon flux density                        (umol photon m-2)
                atten = process_space_real(ipnt(7))  !    attenuation of light by water + plankton Chl           (-)
                ...
                speciesLoop : do iSpec = 1, nrSpec
                    ...
                    alpha = process_space_real(ipnt(spInc + 5))   !      alpha for photosynthesis in protist                    (Figure this out!)
                    ChlCm = process_space_real(ipnt(spInc + 6))   !      maximum cellular Chl:C ratio                           (gChl gC-1)
                    ChlCo = process_space_real(ipnt(spInc + 7))   !      minimum cellular Chl:C ratio                           (gChl gC-1)
                    CR = process_space_real(ipnt(spInc + 8))   !      catabolic respiration quotient                         (-)
                    FrAut = process_space_real(ipnt(spInc + 9))   !      fraction of mortality to autolysis                     (-)
                    FrDet = process_space_real(ipnt(spInc + 10))   !      fraction of mortality to detritus                      (-)
                    KtNH4 = process_space_real(ipnt(spInc + 11))   !      Kt for NH4 transport                                   (gN m-3)
                    KtNO3 = process_space_real(ipnt(spInc + 12))   !      Kt for NO3 transport                                   (gN m-3)
                    KtP = process_space_real(ipnt(spInc + 13))   !      Kt for DIP transport                                   (gP m-3)
                    MrtRT = process_space_real(ipnt(spInc + 14))   !      mortality at reference temperature                     (-)
                    NCm = process_space_real(ipnt(spInc + 15))   !      N:C that totally represses NH4 transport               (gN gC-1)
                    NCo = process_space_real(ipnt(spInc + 16))   !      minimum N-quota                                        (gN gC-1)
                    NCopt = process_space_real(ipnt(spInc + 17))   !      N:C for growth under optimal conditions                (gN gC-1)
                    NO3Cm = process_space_real(ipnt(spInc + 18))   !      N:C that totally represses NO3 transport               (gN gC-1)
                    NO3Copt = process_space_real(ipnt(spInc + 19))   !      N:C for growth on NO3 under optimal conditions         (gN gC-1)
                    PCm = process_space_real(ipnt(spInc + 20))   !      PC maximum quota                                       (gP gC-1)
                    PCo = process_space_real(ipnt(spInc + 21))   !      PC minimum quota                                       (gP gC-1)
                    PCoNCm = process_space_real(ipnt(spInc + 22))   !      maximum NC when PC is minimum (PCu = 0)                (gN gC-1)
                    PCoNCopt = process_space_real(ipnt(spInc + 23))   !      optimum NC when PC is minimum (PCu = 0)                (gN gC-1)
                    PCopt = process_space_real(ipnt(spInc + 24))   !      PC optimum quota                                       (gP gC-1)
                    PSDOC = process_space_real(ipnt(spInc + 25))   !      proportion of current PS being leaked as DOC           (-)
                    Q10 = process_space_real(ipnt(spInc + 26))   !      Q10 for UmRT                                           (-)
                    redco = process_space_real(ipnt(spInc + 27))   !      C respired to support nitrate reduction for NH4        (gC gN-1)
                    relPS = process_space_real(ipnt(spInc + 28))   !      relative PSmax:Umax on phototrophy                     (-)
                    ReUmNH4 = process_space_real(ipnt(spInc + 29))   !      max. growth rate supported by NH4-N:Umax               (-)
                    ReUmNO3 = process_space_real(ipnt(spInc + 30))   !      max. growth rate supported by NO3-N:Umax               (-)
                    RT = process_space_real(ipnt(spInc + 31))   !      reference temperature for UmRT                         (deg C)
                    rProt = process_space_real(ipnt(spInc + 32))   !      radius of nutrient repleted protist cell               (um)
                    UmRT = process_space_real(ipnt(spInc + 33))   !      maximum growth rate at reference T                     (d-1)
                    ...
                    process_space_real(ipnt(spInc + 16)) = NPP
                    process_space_real(ipnt(spInc + 17)) = synChl
                    process_space_real(ipnt(spInc + 18)) = degChl
                    process_space_real(ipnt(spInc + 19)) = totR
                    process_space_real(ipnt(spInc + 20)) = Cu
                    process_space_real(ipnt(spInc + 21)) = mrt
                    process_space_real(ipnt(spInc + 22)) = mrtFrAut
                    process_space_real(ipnt(spInc + 23)) = mrtFrDet
