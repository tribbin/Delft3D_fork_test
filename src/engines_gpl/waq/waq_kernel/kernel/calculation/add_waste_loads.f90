...
module m_add_waste_loads
    ...
    private
    public :: add_waste_loads, add_waste_loads_special
...
contains
    ...
    subroutine add_waste_loads(num_substances_transported, num_substances_total, num_cells, num_exchanges, &
            num_waste_loads, num_waste_load_types, num_monitoring_cells, intopt, idt, itime, &
        ...
        integer(kind = int_wp), intent(in) :: substance_i                    !< First substance in array
        ...
        data       ithandl  /0/
        if (timon) call timstrt ("add_waste_loads", ithandl)
        ! No wasteloads
        ...
        if (substance_i == 1) then
            ...
            if (.not. allocated(ibpoint)) then
                ...
                if (ierr_alloc /= 0) then
                    write(*, *) 'ERROR: allocating work array in add_waste_loads'
            ...
            call setup_user_defined_waste_loads(num_waste_loads, num_substances_total, num_substances_transported, num_cells, syname, &
        ...
        do i = 1, num_waste_loads
            ...
            case (0) ! Original situation
                ...
                if (abs(WasteFlow) <= 1.0E-30) then       ! just load of mass
                    do i1 = substance_i, substance_i + nsys - 1
    ...
    end subroutine add_waste_loads

    !> Special versions of add_waste_loads for integration scheme 24. Adds waste
    !! loads to the derivative vector DERIV, except withdrawals.
    !! Withdrawals are only summed, and evaluated in the transport routine
    !! of integration scheme in locally_adaptive_time_step.
    !! This routine identifies which wasteflows are -999.0 (missing value).
    !! Those flows are replaced by the detected closure error / nr of
    !! waste flows it concerns. This is an inherent weakness of the approach
    !! that all get the same value.
    ...
    !! Surface and bed loads require the presence of parameter SURF.
    !! Bank loads require the presence of parameter LENGTH.
    subroutine add_waste_loads_special (num_substances_transported, num_substances_total, num_cells, num_exchanges, num_waste_loads, &
        ...
        integer(kind = int_wp) :: nrdetec    !< Number of wasteloads detected for missing flow value
        integer(kind = int_wp) :: i, i1      !< Loop counter
        integer(kind = int_wp) :: iwst       !< Index of cell for waste load / withdrawal:
        !< iwst == -1 => over the whole water surface. Values are /m2.
        !< iwst == -2 => along the bank length of a cell. Values are /m.
        !< iwst == -3 => over the whole water bed. Values are /m2.
        integer(kind = int_wp) :: ip         !< Auxiliary variable for 'from' and 'to' cell number
        ...
        9999 if (timon) call timstop (ithandl)
    end subroutine add_waste_loads_special


    !! calls the user defined wasteload routines
    subroutine setup_user_defined_waste_loads(num_waste_loads, num_substances_total, num_substances_transported, &
            num_cells, syname, conc, itime, num_waste_load_types, wastid, wstnam, &
            wsttyp, iwaste, iwtype, waste)
        ...
        if (timon) call timstrt ("setup_user_defined_waste_loads", ithandl)
        ...
        do iwst = 1, num_waste_loads
            ...
            do substance_i = 1, num_substances_total
                wasteloads(iwst)%loads(substance_i) = waste(substance_i, iwst)
            enddo
        enddo

        ! call routine
        call delwaq_user_wasteload(num_waste_loads, wasteloads, num_substances_total, num_substances_transported, &
                num_cells, itime, conc, syname)

        ! updated wasteloads to old delwaq arrays
        do iwst = 1, num_waste_loads
            waste(0, iwst) = wasteloads(iwst)%flow
            do substance_i = 1, num_substances_total
                waste(substance_i, iwst) = wasteloads(iwst)%loads(substance_i)
            enddo
        enddo
    ...
    end subroutine setup_user_defined_waste_loads

end module m_add_waste_loads
