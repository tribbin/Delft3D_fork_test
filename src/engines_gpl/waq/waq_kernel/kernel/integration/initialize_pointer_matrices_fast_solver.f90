...
!!  rights reserved.
module m_initialize_pointer_matrices_fast_solver
    use m_waq_precision
    use timers
    ...
    private
    public :: initialize_pointer_matrices_fast_solver

contains

    !> Create a zero based row pointer and direct mapping 
    !! from iq to the matrix with fmat and tmat for from and to
    subroutine initialize_pointer_matrices_fast_solver(num_cells, num_boundary_conditions, num_exchanges, &
        ...
        integer(kind = int_wp) :: cell_i !< Current volume
        integer(kind = int_wp) :: iq   !< Current edge
        integer(kind = int_wp) :: iadd !< Auxiliary variable

        integer(kind = int_wp) :: ithandl = 0
        if (timon) call timstrt ("initialize_pointer_matrices_fast_solver", ithandl)
        ...
        iadd = 0
        !  in 3D first 2 co diagonals are the vertical
        if (num_exchanges /= num_exchanges_u_dir + num_exchanges_v_dir) iadd = 2

        ! accumulate to pointer start of rows
        rowpnt(0) = 0
        do cell_i = 1, num_cells
            rowpnt(cell_i) = rowpnt(cell_i) + rowpnt(cell_i - 1) + iadd
        enddo
        do cell_i = num_cells + 1, num_cells + num_boundary_conditions
            rowpnt(cell_i) = rowpnt(cell_i - 1)
        enddo
    ...
    end subroutine initialize_pointer_matrices_fast_solver
end module m_initialize_pointer_matrices_fast_solver
