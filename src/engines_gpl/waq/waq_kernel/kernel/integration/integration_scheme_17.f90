...
contains
    ...
    subroutine scheme_17_steady_state_hz_upwind_vl_upwind(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        use m_gmres_utils, only: fill_matrix_hz_backward_diff_vl_backward_diff
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            ithandl = 0
            if (timon) call timstrt("scheme_17_steady_state_hz_upwind_vl_upwind", ithandl)
            ...
            do substance_i = 1, num_substances_transported
                ...
                call fill_matrix_hz_backward_diff_vl_backward_diff(num_cells, num_boundary_conditions, num_exchanges_u_dir, num_exchanges_v_dir, num_exchanges, &
                        j(ixpnt:), nddim, nvdim, j(idpnw:), j(ivpnw:), &
                        a(iarea:), a(iflow:), a(idisp:), a(idnew:), a(ivnew:), &
                        substance_i, fast_solver_arr_size, gm_amat(1, ith), j(imat:), rowpnt, &
    ...
    end subroutine scheme_17_steady_state_hz_upwind_vl_upwind
