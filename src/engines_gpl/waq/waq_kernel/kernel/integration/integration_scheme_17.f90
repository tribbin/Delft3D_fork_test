    ...
    subroutine integration_scheme_17(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            do isys = 1, num_substances_transported
                ...
                call dlwqh2(num_cells, num_boundary_conditions, num_exchanges_u_dir, num_exchanges_v_dir, num_exchanges, &
                            j(ixpnt:), nddim, nvdim, j(idpnw:), j(ivpnw:), &
                            a(iarea:), a(iflow:), a(idisp:), a(idnew:), a(ivnew:), &
                            isys, fast_solver_arr_size, gm_amat(1, ith), j(imat:), rowpnt, &
