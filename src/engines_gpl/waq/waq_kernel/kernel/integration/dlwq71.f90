...
contains
    ...
    subroutine dlwq71(disp, disper, area, flow, aleng, &
        ...
        real(kind = real_wp),   intent(in   ) :: disper(*)      !< Additional dispersion array (NODISP*NOQ)
        real(kind = real_wp),   intent(in   ) :: area(*)        !< Exchange surface area (NOQ)
        real(kind = real_wp),   intent(in   ) :: flow(*)        !< Flows accross exchange surfs (NOQ)
        real(kind = real_wp),   intent(in   ) :: aleng(*)       !< From- and to lengths (2*NOQ)
        real(kind = real_wp),   intent(in   ) :: velo(*)        !< Additional velocity array (NOVELO*NOQ)
        real(kind = real_wp),   intent(in   ) :: conc (*)       !< Concentrations (NOTOT*NOSEG)
        ...
        integer(kind = int_wp), intent(in   ) :: noq1           !< Nr of exchanges in first dir.
        integer(kind = int_wp), intent(in   ) :: noq2           !< Nr of exchanges in second dir.
        ...
        integer(kind = int_wp), intent(in   ) :: idpnt(*)       !< Pointer systems to dispersions (NOSYS)
        integer(kind = int_wp), intent(in   ) :: ivpnt(*)       !< Pointer systems to velocities (NOSYS)
        ...
        real(kind = real_wp), intent(inout)   :: amass2(*)      !< Mass balance array (NOTOT*5)
        integer(kind = int_wp), intent(in   ) :: ilflag         !< If 0 then 3 length values
        real(kind = real_wp),   intent(inout) :: dmpq(*)        !< Mass balance dumped exchange (NOTOT*NDMPQ*?)
                                                                !< If INTOPT > 7
