...
contains
    ...
    subroutine scheme_5_time_explicit_flux_corrected_transport(buffer, file_unit_list, file_name_list, &
            action, dlwqd, gridps)
        ...
        use m_flux_corrected_transport_fct, only: apply_fct_boris_book_5_12_14, first_step_fct
        ...
        ! Local variables
        logical :: imflag                 !< True if monitoring took place, set in write_output, used in set_cumulative_arrays_zero
        logical :: ihflag                 !< True if history    took place, set in write_output, used in set_cumulative_arrays_zero
        logical :: idflag                 !< True if dump       took place, set in write_output, not used
        logical :: lrewin                 !< True if rewind     took place, set in update_volumes_and_time_step, used for closure error corr.
        logical :: ldumm2                 !< Dummy logical, parameter in update_time_dependent_external_forcing
        real(kind = real_wp) :: rdummy(1) !< Dummy real in update_time_dependent_external_forcing
        ...
        integer(kind = int_wp) :: substance_i    !< Loop counter substances (loop should be in a called subroutine !)
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            if (action == ACTION_FINALISATION) then
                ...
                if (timon) call timstrt("scheme_5_time_explicit_flux_corrected_transport", ithandl)
                goto 20
            end if
        ...
        dlwqd%iaflag = iaflag
        dlwqd%itime = itime
    end subroutine scheme_5_time_explicit_flux_corrected_transport
