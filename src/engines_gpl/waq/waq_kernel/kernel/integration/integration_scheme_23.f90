...
contains
    ...
    subroutine scheme_23_time_explicit_leonards_quickest(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)

        use m_scheme_23_utils, only: make_explicit_derivatives_leonard
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            if (action == ACTION_FINALISATION) then
                ...
                if (timon) call timstrt("scheme_23_time_explicit_leonards_quickest", ithandl)
                goto 20
            end if
            ...
            call make_explicit_derivatives_leonard(num_substances_transported, num_substances_total, num_cells, num_exchanges_u_dir, num_exchanges_v_dir, &
                    num_exchanges_z_dir, num_exchanges, nddim, nvdim, a(idisp:), &
        ...
        dlwqd%iaflag = iaflag
        dlwqd%itime = itime
    end subroutine scheme_23_time_explicit_leonards_quickest
