...
contains
    ...
    subroutine scheme_13_time_explicit_space_upwind(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        use m_calculate_new_volumes, only: calculate_new_volumes
        use m_double_sweep_solver, only: double_sweep_solver_13_14
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            if (action == ACTION_FINALISATION) then
                ...
                if (timon) call timstrt("scheme_13_time_explicit_space_upwind", ithandl)
                goto 20
            end if
            ...
            ! add the waste loads
            call add_waste_loads(num_substances_transported, num_substances_total, nosss, num_exchanges, num_waste_loads, &
            ...
            call calculate_transport_with_explicit_derivatives(num_substances_transported, num_substances_total, nosss, num_exchanges_u_dir, num_exchanges_v_dir, &
            ...
            call double_sweep_solver_13_14(num_substances_transported, num_substances_total, nosss, num_exchanges_z_dir, lnoq, &
        ...
        dlwqd%iaflag = iaflag
        dlwqd%itime = itime
    end subroutine scheme_13_time_explicit_space_upwind
