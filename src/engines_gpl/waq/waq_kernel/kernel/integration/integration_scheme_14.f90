...
contains
    ...
    subroutine scheme_14_time_explicit_flux_corrected_transport(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            if (action == ACTION_FINALISATION) then
                ...
                if (timon) call timstrt("scheme_14_time_explicit_flux_corrected_transport", ithandl)
                goto 20
            end if
        ...
        dlwqd%iaflag = iaflag
        dlwqd%itime = itime
    end subroutine scheme_14_time_explicit_flux_corrected_transport
