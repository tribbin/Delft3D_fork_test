...
contains
    ...
    subroutine scheme_25_emission_time_explicit_space_upwind(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        use m_calculate_transport_with_explicit_derivatives, only: calculate_transport_with_explicit_derivatives_special
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            if (ACTION == ACTION_FINALISATION) then
                ...
                if (timon) call timstrt("scheme_25_emission_time_explicit_space_upwind", ithandl)
                goto 20
            endif
            ...
            IF (ACTION == ACTION_INITIALISATION) THEN
                if (timon) call timstrt("scheme_1_time_explicit_space_backward", ithandl)
            ...
            call calculate_transport_with_explicit_derivatives_special(num_substances_transported, num_substances_total, nosss, num_exchanges_u_dir, num_exchanges_v_dir, &
        ...
        dlwqd%iaflag = iaflag
        dlwqd%itime = itime
    end subroutine scheme_25_emission_time_explicit_space_upwind
