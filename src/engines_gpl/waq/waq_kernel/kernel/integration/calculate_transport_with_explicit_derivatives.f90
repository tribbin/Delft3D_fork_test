...
!!  rights reserved.
module m_calculate_transport_with_explicit_derivatives
    use m_waq_precision
    use timers
    ...
    private
    public :: calculate_transport_with_explicit_derivatives, calculate_transport_with_explicit_derivatives_special
...
contains
    ...
    subroutine calculate_transport_with_explicit_derivatives(num_substances_transported, num_substances_total, &
            num_cells, num_exchanges_u_dir, num_exchanges_v_dir, num_exchanges_z_dir, num_exchanges, &
            num_dispersion_arrays, num_velocity_arrays, disp, &
        ...
        if (timon) call timstrt ("calculate_transport_with_explicit_derivatives", ithandl)
        ...
        do iq = 1, num_exchanges
            ...
            do substance_i = 1, num_substances_transported
                ...
                ! upwinding
                if (v > 0.0) then
                    dq = (v + d) * conc(substance_i, ifrom) - d * conc(substance_i, ito)
                else
                    dq = (v - d) * conc(substance_i, ito) + d * conc(substance_i, ifrom)
                end if
            ...
            20    do substance_i = 1, num_substances_transported
                ...
                ! upwinding
                if (v > 0.0) then
                    dq = (v + d) * bound(substance_i, -ifrom) - d * conc (substance_i, ito)
                else
                    dq = (v - d) * conc (substance_i, ito) + d * bound(substance_i, -ifrom)
                end if
                deriv(substance_i, ito) = deriv(substance_i, ito) + dq
            ...
            40    do substance_i = 1, num_substances_transported
                ...
                ! upwinding
                if (v > 0.0) then
                    dq = (v + d) * conc(substance_i, ifrom) - d * bound(substance_i, -ito)
                else
                    dq = (v - d) * bound(substance_i, -ito) + d * conc(substance_i, ifrom)
                end if
                deriv(substance_i, ifrom) = deriv(substance_i, ifrom) - dq
    ...
    end subroutine calculate_transport_with_explicit_derivatives

        !> Calculates transport with explicit derivatives for the advection diffusion equation. Special
    !! version of calculate_transport_with_explicit_derivatives for emission module (25) with total flux equal to flow for this substance  (dq = v).
    !! Besides the (main) water flow in the array FLOW(num_exchanges), there are optional additional velocities.
    !! These options are often used in the vertical for settling velocities of particulates or
    !! floating velocities of blue-green algae.
    !! Next to the constant diffusion terms in 3 directions contained in DISP(3), optionally
    ...
    !! Furthermore the fluxes in and out of monitoring areas for detail balances are accumulated on
    !! the fly. IQDMP(num_exchanges) indicates Which flux needs to be accumulated in what balance.
    subroutine calculate_transport_with_explicit_derivatives_special(num_substances_transported, num_substances_total, num_cells, num_exchanges_u_dir, num_exchanges_v_dir, &
        ...
        if (timon) call timstop (ithandl)
    end subroutine calculate_transport_with_explicit_derivatives_special

end module m_calculate_transport_with_explicit_derivatives
