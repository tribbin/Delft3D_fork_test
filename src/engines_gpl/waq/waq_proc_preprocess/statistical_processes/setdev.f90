...
contains
    ...
    SUBROUTINE SETDEV (LUNREP, NOKEY, &
            KEYNAM, KEYVAL, &
            PERNAM, PERSFX, &
            PSTART, PSTOP, &
            IPROC, aProcesProp, &
        ...
        !
        !     FUNCTION            : Sets io list for statistical routine STADEV
        !
        ...
        if (timon) call timstrt("setdev", ithndl)
        !
        !     init
        !
        ...
        IF (IERR_ALLOC /= 0) THEN
            ...
            WRITE(LUNREP, *) 'in routine SETDEV_3, buffer length:', NOKEY
        ...
        !
        aProcesProp%name = 'STADEV'
        WRITE(aProcesProp%name(7:10), '(I4.4)') IPROC
        aProcesProp%routine = 'STADEV'
        aProcesProp%text = 'mean and standard deviation'
        ...
        IF (IERR_ALLOC /= 0) THEN
            ...
            WRITE(LUNREP, *) 'in routine SETDEV_1, array length:', aProcesProp%no_input, aProcesProp%no_output
        ...
        WRITE(LUNREP, 2000) 'Statistical output named [', aItemProp%name, &
                '] created with average from [', aProcesProp%input_item(1)%name, ']'
        !
        IF (SUFFIX(1:ISLEN) /= ' ') THEN
        ...
        ENDIF
        aItemProp%default = -999.
        aItemProp%text = 'standard deviation ' // aProcesProp%input_item(1)%name
        aItemProp%waqtype = WAQTYPE_NONE
        iret = ItemPropCollAdd(AllItems, aItemProp)
