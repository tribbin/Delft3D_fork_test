...
contains
   ...
   subroutine dlwqp1(file_unit_list, file_name_list, &
      ...
      character(:), allocatable :: share_dir
      ...
      call open_waq_files(file_unit_list(35), file_name_list(35), 35, 1, ierr)
      ...
      if (.not. swi_nopro) then
         if (get_command_argument_by_name('-p', pdffil, parsing_error)) then
            ...
            if (parsing_error) then
               write (line, '(a)') ' Could not read process library file name after -p argument.'
               call write_log_message(line)
               call stop_with_error()
            end if
            inquire (file=pdffil, exist=lfound)
            if (lfound) then
               write (line, '(a)') ' Using specified process library file: '//trim(pdffil)
               call write_log_message(line)
            else
               write (line, '(a)') ' Specified process library file does not exist: '//trim(pdffil)
               call write_log_message(line)
               call stop_with_error()
            end if
         else
            pdffil = trim(share_dir)//'proc_def.dat'
            inquire (file=pdffil, exist=lfound)
            if (lfound) then
               write (line, '(a)') ' Using default Process library file: '//trim(pdffil)
               call write_log_message(line)
            else
               write (line, '(a)') ' Default process library file does not exist: '//trim(pdffil)
               call write_log_message(line)
               call stop_with_error()
            end if
         end if
      ...
      searchdefaultblmfil = .false.
      if (is_command_arg_specified('-eco')) then
         l_eco = .true.
         line = ' '
         call write_log_message(line)
         ...
         if (get_command_argument_by_name('-eco', blmfil, parsing_error)) then
            inquire (file=blmfil, exist=lfound)
            if (lfound) then
               write (line, '(a)') ' Using specified BLOOM species definition file: '//trim(blmfil)
               call write_log_message(line)
            else
               write (line, '(a)') ' Specified BLOOM species definition file does not exist: '//trim(blmfil)
               call write_log_message(line)
               call stop_with_error()
            end if
         else
            searchdefaultblmfil = .true.
         end if
      else
         ...
         if (blm_act > 0 .and. .not. swi_nopro) then
            ...
            call write_log_message(line)
            write (line, '(a)') ' Will try using default eco input file.'
            call write_log_message(line)
            searchdefaultblmfil = .true.
         else
            l_eco = .false.
      ...
      if (searchdefaultblmfil) then
         blmfil = trim(share_dir)//'bloom.spe'
         inquire (file=blmfil, exist=lfound)
         if (lfound) then
            write (line, '(a)') ' Using default BLOOM species definition file: '//trim(blmfil)
            call write_log_message(line)
         else
            write (line, '(a)') ' Default BLOOM species definition file does not exist: '//trim(blmfil)
            call write_log_message(line)
            call stop_with_error()
         end if
      end if
      ...
      if (l_eco) then
         open (newunit=lunblm, file=blmfil, status='old', iostat=ierr)
         if (ierr /= 0) then
...
100         continue
      ...
      call open_waq_files(file_unit_list(2), file_name_list(2), 2, 2, ierr)
      ...
      if (nmis > 0) then
         call open_waq_files(file_unit_list(24), file_name_list(24), 24, 1, ierr)
      ...
      call open_waq_files(file_unit_list(25), file_name_list(25), 25, 1, ierr)
