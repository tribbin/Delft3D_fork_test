...
!
!           The mixture of the transport layer is determined on a
!           rectangular cross-section. So fill array grsize(dmed,igr,1)
!           in subroutine GSDMED for the second time.
!
...
!                           after call of Gsdhgi the Sediment exponent
   ...
   enddo
!
!     Calculate layer thickness and dune height and length when a
!     steady flow has been calculated.
!
   call gsdula (ngrid  ,nbran  ,g      ,gsopts ,branch ,&
   &sedpar ,cp     ,afwfqs ,trform ,duncon ,&
   &grsize(1,1,1)  ,sedexp ,duneh  ,dunel  ,deff    )
!
!     De dikte van de transportlaag is juist bepaald. Bepaal hierbij de
!     definitieve waarde van N(i) en dzr(i) bij level zb(i)-deff(i,2)
!     en de nieuwe samenstelling van ptrla(i,j,2).
!     Indien ook een exchangelaag aanwezig, dan deze procedure herhalen
!     en de nieuwe samenstelling van pexla(i,j,2).
!
   if (nunlay .gt. 1) then
!
      thexfp = 1.0
...
!
!     Finally the initial mixture is determined so calculate the characteristic
!     grain sizes.
!
   do ibr=1,nbran
   ...
   enddo
!
!     Set array with previous characteristic diameters equal to
!     array with current grain sizes.
!
   do 120 igr=1,ngrid
