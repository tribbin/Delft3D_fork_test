...
!
! Module:             SOWRBF (SObek WRite BuFfer)
!
! Module description: Routine writes buffered information if the model
!                     crashes due to drying or exceeding of maximum
!                     number of iterations.
!                     Also in case of high Froude numbers, the buffered
!                     Froude numbers are written.
!
!                     In case of continuation of the computation after
!                     exceeding of the maximum number of iterations
!                     the amount of information written to file will
!                     be limited.
!
!                        [ residuals:  max 20 times
!                          structures: max 10 times
!                          solution:   max  4 times
!                          Froude:     max 10 times ]
!
!-----------------------------------------------------------------------
! Parameters:
! NR NAME              IO DESCRIPTION
! 13 conv              I  Switch to indicate convergence of solution:
!                         = 0 no convergence yet
!                         = 1 convergence obtained
!                         = 2 no convergence obtained after exceeding
!                             the maximum number of iterations and
!                             the computation will be continued
! 15 frobuf(8)         IO Buffer to determine the maximum Froude
!                         number during the computation. The buffer
!                         contains the current maximum:
!                         (1) Maximum Froude number in cel,
!                         (2) on time step,
!                         (3) on iteration step,
!                         (4) at grid point.
!                         (5-8) The same but for grid points.
!  4 ibuf(5)           IO Contains pointers in circular buffers:
!                         (1) Pointer for buffers with residuals and
!                             structure data.
!                             (A minus means that the solution buffer
!                             has been written in this step.)
!                         (2) Pointer for buffers with solutions
!                         (3) Pointer for Froude buffers
!                         (4) Counter for number of written buffers
!                             (residuals)
!                         (5) Counter for number of written Froude
!                             buffers
!  1 jufrou            I  dump file handler of froude numbers
!  1 juresd            I  dump file handler of results
!  2 justrd            I  dump file handler of structure data
!  3 jusold            I  dump file handler of solutions
! 12 ker               I  Error code:
!                         ok     (0) : No error
!                         info   (1) : Informative message
!                         warnng (2) : Warning
!                         fatal  (3) : Fatal error (processing stops)
! 14 lfrou             I  Flag if Froude numbers during simulation are
!                         high
!  9 ngrid             I  Number of grid points in network.
!  7 nstru             I  Number of structures
!  5 resbuf(dmbuf1,6)  I  Buffer with latest residues
!                         (,1,) : time step    (,2,) : iteration step
!                         (,3,) : Delta h      (,4,) : grid point nr
!                         (,5,) : Delta q      (,5,) : grid point nr
! 11 solbuf
!    (dmbuf2,7,ngrid)  IO Buffer with latest solutions or Froude nrs
!                         *  Solutions, for all grid points:
!                         Index 1 : buffer number
!                         Index 2 : Quantity number
!                         (,1,)   : h            (,2,) : q
!                         (,3,)   : h above slot (,4,) : width flow
!                         (,5,)   : area flow    (,6,) : C**2*R
!                         Index 3 : grid point number
!                         *  High Froude numbers on index (,7,):
!                         Index 1 : Froude numbers or identification:
!                         Time info:
!                         (1,,) : time frame pointer (2,,) : -1
!                         (3,,) : time step          (4,,) : iteration step
!                         Froude numbers:
!                         (1,,) : time frame pointer (2,,) :grid point nr
!                         (3,,) : Froude nr in cel
!                         (4,,) : Froude nr in grid point
!                         Index 3 : Frame index
!                         *  Empty
!                         (2,,) : 0
!  6 strbuf
!    (dmbuf1,2,nstru)  I  Buffer with latest structure data
!                         (,1,) : structure discharge
!                         (,2,) : structure head
...
!
! $Log: sowrbf.pf,v $
! Revision 1.5  1999/03/15  15:03:32  kuipe_j
! Improve Froude file and Dumpfiles
!
! Revision 1.4  1998/12/11  13:07:31  kuipe_j
! improve annotation in dumps
!
! Revision 1.3  1998/06/08  13:15:42  kuipe_j
! time lag hydr controller
!
! Revision 1.2  1997/11/04  14:11:48  kuipe_j
! Bug structure in lat Q
!
! Revision 1.1  1997/01/23  08:30:18  kuipe_j
...
!
!     Determine if data was written to the buffers in this
!     iteration step and the iteration step number was also updated
!
      if (ibuf(1).lt.0) then
         ibuf(1) = -ibuf(1)
         noupda = .true.
!        no update, one residual less will be printed (the oldest),
!        as its step annotation is wrong.
         nresb = 19
      else
         noupda = .false.
         nresb = 20
...
!
!    Determine maximum Froude numbers
!
...
!           Maximum Froude number in gridcel
!               froude
...
!           Maximum Froude number in gridpoint
!               froud1
