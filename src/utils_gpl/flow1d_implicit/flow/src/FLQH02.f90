...
!
! Module:             FLow QH relation type 02 (open flume)
!
...
!                        Function: Discharge through structure type 2
!                        Updates: None
...
! History:
! $Log: flqh02.pf,v $
! Revision 1.3  1999/03/15  15:50:30  kuipe_j
! tabs removed
!
! Revision 1.2  1998/06/08  12:35:36  kuipe_j
! log added
!
!
!
...
! strsta    I      structure status
! strclo    I      structure status
! width     I      width of flume (W)
! le        I      bottom level at the entrance (Le)
! z         I      difference between inlet and outlet levels (z)
! length    I      length of flume (L)
! alpha     I      ratio width of approach section vs. W (alfa)
! lw        I      length of approach section (Lw)
! ksa       I      roughness of approach section (ksa)
! ks        I      roughness of flume section (ks)
! rloc      I      inlet/outlet rounding (r)
! wloc      I      inlet/outlet beveling (w)
! theta     I      bevel angle (theta)
! cc(1)     I      coefficient C1 (if = 0 then calculate)
! cc(2)     I      coefficient C2 (if = 0 then calculate)
! cc(3)     I      coefficient C3 (if = 0 then calculate)
! nflume    I      number of flumes (Nf) (=1)
! beta      I      correction factor (beta)
! bot       I      average bed level
! lbvin     I      inlet bevelled (T), round (F)
! lbvout    I      outlet bevelled (T), round (F)
...
!
! to avoid an error here in the code set nflume to 1
   nflume = 1
...
!
! calculate defaults for c1, c2 and c3
   kco1 = 1
   do 20 i = 1, 3
      if (EQUAL(cc(i),0.)) then

!
! has cc(1) already been calculated?
         if (kco1 .eq. 0) then
            cc(i) = codef
         else
            x  = delh1 / width
...
!
! the following if statement is no longer used.
!---  De eerste keer per iteratie worden SCRIT en het flowtype bepaald:
!      if (k .eq. 0) then

!
! calculate rcrit and acrit for scrit
   r1 = beta * (hup - bot)
...
!
! flowtype 1: critical depth at inlet
1  r1 = beta * (hup - bot)
   ri = h1le * width / (2. * h1le + width)
   hf1i = FLFR1I (width, q, lw, r1, ksa, alpha, ri, ks, h1le, ski)
   ycrit = yci
...
!
! flowtype 2: critical depth at outlet
2  r1 = beta * (hup - bot)
   ai = h1le * width
   ri = ai / (2.*h1le + width)
   hf1i = FLFR1I (width, q, lw, r1, ksa, alpha, ri, ks, h1le, ski)
...
!
! use initial value for yco, where hfiostart = l*q*q/ki/ki

   hfio = length * q * q / (ski * ski)
   yco  = 2./3. * (delhz - hf1i - hfio)

!
! calculate ro by iteration
   do 11 i = 1, 10
...
!
! flowtype 3: subcritical flow in flume
3  r1 = beta * (hup - bot)
   ai = h1le * width
   ri = ai / (2.*h1le + width)
