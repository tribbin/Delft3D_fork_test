...
!
! Module:             FLow QH relation type 04 (culvert)
!
...
!                        Programmer: G. van Driel / H. van Zanten
!                        Function: Discharge through structure type 4
!                        Updates: None
!                     Note: due to array length limitations, part of
!                     the functionality has been left out. This is to
!                     included later with the introduction of dynamic
!                     array allocation. See documentation for details.
!
!
! an initial value is required for yco0 (this is not in the
! original WENDY code and needs to be checked!!!
!
!-----------------------------------------------------------------------
! Subprogram calls:
!
...
! History:
! $Log: flqh04.pf,v $
! Revision 1.2  1998/06/08  12:35:38  kuipe_j
! log added
!
!
!
...
!
! set round or rectangular culvert
   lround  = .true.
   if (width .gt. 0.) then
      lround = .false.
   endif
...
!
! set beveling or rounding at entrance and exit
! currently only the beveling option is available
   lbevup = .true.
   lbevdn = .true.

!
! set number of culverts to one
   nculv   = 1
   ...
   rlemax  = MAX(le,(le-z))
!
! check if energy level is lower dan MAX(inlet level,outlet level)
...
!--De eerste keer per iteratie wordt het stroomtype bepaald (lange lus!)
! this loop is no longer used
!      if (k .eq. 0) then

!
! determine scrit for each culvert type
! round culvert
   if (lround) then

! iteratief yi bepalen uit yi=H1-Le-Q*Q/2g/Ai/Ai (1x per iter.)
! divergentie als :
!    |(deltayi(i+1)/deltayi(i))| > 1.
!             of
!    |(deltayi(i+1)/yi(i+1))| > 1.e-02 na 10 iteratie stappen
! met deltayi(i+1) = yi(i+1) - yi(i)
! in geval van divergentie kies yi = yci (met verwaarlozing van hf1i)

      term = qin * qin / (2. * g)

!
! free or drowned flow upstream?
...
!
! define xcrtm and ycrtm to determine criterium
      if (z .lt. 0.) then
...
! iteratief yi uit yi=H1-Le-Q*Q/2g/(yi*W)/(yi*W)  (1x per iter.)
! divergentie als :
!    |(deltayi(i+1)/deltayi(i))| > 1.
!             of
!    |(deltayi(i+1)/yi(i+1))| > 1.e-02 na 10 iteratie stappen
...
!
! define ycrtm to determine criterium
      if (z .lt. 0.) then
         ycrtm = yci + 2./3 * z
      else
         ycrtm = yci
...
!
! calculate values for c1-6
! within 1 iteration step the coefficients are kept constant
! c1,c2,c3
   kco1 = 1
   do 20 i=1,3
...
!-- iteratief bepalen van hfio, start met beginwaarde voor yco, waarbij
!---  hfiostart = l*q*q/ki/ki
!
! determne hfio by iteration
...
!
! check for divergence
! yco0 is not yet defined. The assumption is that the
! following IF statement is not true for the first
! iteration.
...
112      continue
         ...
         aco = aco1
!
! this is an odd statement! Is it correct?
         yco = yco
1125     continue
...
!---         iteratief bepalen van hfio start met beginwaarde voor yco,
!---         waarbij hfiostart = l*q*q/ki/ki
!
! determine hfio by iteration
            hfio = cullen* qin * qin / (sk1i*sk1i)
            yco  = (delhz-hf1i-hfio)*2./3.
