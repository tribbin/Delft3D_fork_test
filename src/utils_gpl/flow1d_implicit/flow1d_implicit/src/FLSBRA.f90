...
!
! Module:             FLSBRA (FLow Substitute Back for Retention Areas)
!
! Module description: Subroutine FLSBRA uses the new hydraulic
!                     parameters calculated in FLBRAN to calculate dQ
!                     and dH at the next time level. This is achieved
!                     by substituting back into the equations for the
!                     structures and the retention area.
!
!-----------------------------------------------------------------------
! Parameters:
...
! History:
! $Log: flsbra.pf,v $
! Revision 1.1  1997/11/04  14:17:30  kuipe_j
! Retention basin
!
!
!
...
!
   ...
   do 10 istat = 1, nqlat
      ...
      if (type .eq. cqlret) then
         ...
         dtpse = qlat(istat,8)
!
!         For back substitution use Eq 9 of memo of Borsboom Oct. 1997
!
!                  S
...
!         Shift to next iteration step (M=m+1)
!         Qr_n+1,M-1      Qr_n+1,m
         qlat(istat,2) = qlat(istat,1)
!         Qr_n+1,M        Qr_n
         qlat(istat,1) = qlat(istat,3) + dq
!
!         For back substitution use Eq 7 of memo of Borsboom Oct. 1997
!
!                Qr_n            S
